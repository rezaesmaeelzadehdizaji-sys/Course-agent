...
function buildSection3() {
  return {
    ...
    children: [
      ...
      para('The lungs of the chicken are unlike ours. They don\'t swell and shrink. Birds instead have nine air sacs that act like bellows to push air steadily one way through the lung tissue. Air flows through tiny tubes called parabronchi, and the important thing is that fresh air continues to flow through during both inhalation and exhalation. That\'s why chickens extract more oxygen out of each breath than we do. It\'s a more efficient system, which is important when you\'re dealing with fast growing birds that need a lot of oxygen [2].'),
      ...
      ...embedPng(imgFile('medullary bone.png'), 'Photo 3.3: Medullary bone in a chicken. During lay, calcium is mobilized from the medullary bone to form strong eggshells. Source: CPC Short Courses.'),
...
function buildSection5() {
  return {
    ...
    children: [
      ...
      para('Commercial white-egg layers like Hy-Line W-36 and other Leghorn-type birds usually hit their stride around 26 to 28 weeks of age, when they are laying very close to one egg per hen per day under good management [29]. Brown-egg strains used in Canada, such as Lohmann Brown-Classic and Hy-Line Brown, follow a similar timeline and are also bred to peak in the mid-90s or better for hen-day production when they are managed and fed well [28,29].'),
      para('In Canada, most farms now plan for roughly a year of production from a flock, about 50 to 52 weeks of lay, so birds are often kept through to around 68 to 70 weeks of age in the first cycle if production and shell quality stay strong [25]. Instead of using a hard "we ship at 72 weeks" rule, the real decision point is whether the birds are still paying their way: good egg numbers, strong shells, acceptable mortality, and reasonable feed conversion. When those pieces start slipping, that is when you look at whether it makes more sense to ship the flock and bring in new pullets.'),
      para('In theory, you can induce a molt to "reset" hens and get another round of lay with better shells, and both white-egg and brown-egg strains will respond, but in Canada this is not something we rely on as a normal tool. Old-style forced molting by taking away feed or water is not acceptable here, and national veterinary guidance says molt should only be considered in unusual situations, under a vet and nutritionist, and using humane, non-fasting methods if it is used at all [26,27].'),
