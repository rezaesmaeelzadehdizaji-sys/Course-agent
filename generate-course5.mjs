...
// Embedded JPEG photo + italic caption (naturalW/naturalH = pixel dims of source file)
function photo(buf, caption, naturalW, naturalH, widthIn = 5.8) {
  if (!buf) return [];
  const dpi = 96;
  let wpx = Math.round(widthIn * dpi);
  let hpx = Math.round(wpx * naturalH / naturalW);
  // Cap portrait images: never exceed 4.5 inches height
  const maxH = Math.round(4.5 * dpi);
  if (hpx > maxH) { hpx = maxH; wpx = Math.round(hpx * naturalW / naturalH); }
  return [
    new Paragraph({
...
// Placeholder cell for photos not yet available — includes photo brief for CPC team
...
function buildSection1() {
  return {
    ...
    children: [
      ...
      para('Broilers are one of the most feed-efficient animals in commercial production, and that kind of efficiency matters to the farm budget and to the environment [3,4]. On one farm, a little wasted feed, a small water leak or poor litter use might not seem like much. But those little losses add up quickly over millions of birds in Canada; higher costs, more waste and greater impact on the environment. That is why farmers are asked to use feed, water, energy and litter wisely, and manage manure, bird mortalities and wash water properly [1,2].'),

      h2('1.1  Understanding the Environmental Footprint'),
      para('Global livestock agriculture contributes roughly 14.5% of all human-caused greenhouse gas emissions, and the poultry sector accounts for about 8% of that total [2]. Per kilogram of meat, broilers produce far less greenhouse gas than beef, pork, or lamb [5]. But being cleaner than beef is not the same as having no footprint, especially at Canadian production volumes.'),
      para('The environmental concerns in commercial poultry farming are not complicated or hard to find. They follow the inputs and the outputs. Feed, water, and energy go in. Meat, manure, ammonia, and wastewater come out. How well you manage those flows determines whether you are adding to those problems or managing them [1].'),
      ...photo(photoBuf('photo1_1.jpg'), 'Photo 1.1: A commercial broiler barn housing birds raised for meat in Canada. Large-scale controlled-environment housing allows year-round production, but efficiently managing inputs and outputs at this scale is the core challenge of sustainable poultry farming. Photo: Chicken Farmers of Canada, CC BY 2.0.', 1024, 731),
      ...image(figBuf('fig1_1.png'), 'Figure 1.1: Environmental footprint of a commercial broiler farm. Major inputs (feed, water, energy, chicks) flow through the production system; outputs include saleable meat, manure with fertilizer value, ammonia and greenhouse gas emissions, and wastewater that requires management to prevent contamination.'),

      h2('1.2  Water: Use and Contamination Risk'),
      para('Water is used in two ways on a poultry farm: the birds drink it, and the farm uses it for cleaning and cooling. Broilers consume roughly 1.6 to 1.8 times their feed intake by weight in water at normal barn temperatures. In hot weather, that ratio climbs above 2:1 [6]. A 20,000-bird broiler barn at peak consumption can use tens of thousands of liters per day. That is not a small number, and in regions where groundwater or municipal water costs are rising, it matters to the bottom line.'),
      para('The bigger risk is what happens to the water once it leaves the farm. Runoff from litter storage, uncovered piles, and land application can carry nitrogen, phosphorus, and pathogens into nearby streams and groundwater [1]. Phosphorus in particular does not flush out of soil quickly. Once it reaches a ditch or waterway it feeds algae blooms that pull oxygen out of the water and kill fish. Provincial environmental regulations on buffer zones, setbacks, and nutrient management planning exist for exactly that reason.'),
      para('Leaking drinker lines are more than just a nuisance. A single loose nipple connection can drip hundreds of liters per day without anyone noticing, and wet litter is one of the most common and expensive problems in broiler production. A dripping nipple is a direct line to a litter problem [6,7].'),
      ...image(photoBuf('Photo1-3-leaking nipple-wet-litter.png'), 'Photo 1.2: A nipple drinker dripping onto the litter floor in a commercial broiler barn, creating a saturated dark patch directly beneath the line. A single loose connection can run for days undetected, raising local ammonia and footpad risk for every bird in the area. Daily water meter readings catch this before it becomes a litter and performance problem [6,7]. Source: CPC Short Courses.'),

      h2('1.3  Soil: Nutrients, Runoff, and Land Management'),
      para('Poultry manure is one of the most nitrogen- and phosphorus-dense organic materials available to Canadian crop farmers [8]. Handled well, it is a genuine asset. A typical broiler litter analysis run on material removed from the barn shows 2.5 to 3.5 percent nitrogen, 2.0 to 3.5 percent phosphorus pentoxide, and 2.0 to 3.5 percent potassium oxide [8]. Applied at the right rate to the right fields at the right time, litter reduces fertilizer costs, improves soil organic matter, and builds long-term soil health.'),
      para('Handled badly, those same nutrients become a liability. Put too much on and the nitrogen washes into groundwater. Phosphorus builds up in the soil and eventually finds its way into a ditch or stream where it feeds algae blooms that kill fish. Spread it before rain or onto frozen ground and most of it runs off before the crop can use any of it [1]. The fix is straightforward: get a manure analysis, know what the crop can actually take, and apply accordingly. It does not have to be complicated, but it has to be done.'),

      h2('1.4  Air Quality: Ammonia, Dust, and Odor'),
      para('Ammonia is the single biggest air quality problem in a poultry barn, and it affects your birds, your own health, and the people around your operation [9]. By the time you can smell it, it is already working against you. Around 25 ppm, the birds\' airways start to get irritated, their natural defenses weaken, and they become more prone to respiratory disease. Keep levels high and you will see it in your performance: studies report body weight losses of 15 to 20 percent in flocks exposed to sustained high ammonia compared to well-ventilated barns [9]. Anyone spending long hours in a barn with a persistent ammonia problem is also taking on a real health risk over time [9].'),
      para('It is not just a barn problem. Ammonia that leaves your building travels. Neighbours smell it before an inspector ever shows up, and in some regions persistent odor complaints have pushed regulators to tighten rules on stocking density, storage, and when litter can be applied [9].'),
      para('Dust from dry litter and feathers is the other piece of this. It carries bacteria and irritants that can drift well past the barn walls. You can cut it down with good litter management, but you cannot eliminate it completely. Ventilation is your main tool for keeping both ammonia and dust under control [10,11].'),
      ...image(figBuf('fig1_2.png'), 'Figure 1.2: Ammonia in the broiler barn. Manure breaks down to release NH3, with wet litter, high temperature, and poor ventilation all accelerating the process. At greater than 25 ppm birds become more susceptible to respiratory disease; above 50 ppm performance drops measurably. Outside the barn, the same ammonia drives odor complaints and contributes to nitrogen buildup in the surrounding environment.'),
      pageBreak(),
    ],
  };
}
...
function buildSection2() {
  return {
    ...
    children: [
      ...
      para('On the farm, efficiency and sustainability are the same thing. Every kilogram of feed that does not turn into bird weight is money out the door. Every liter of water that leaks before a bird drinks it is a cost for nothing. Every hour your heaters run longer than needed because of a gap in the wall is cash you paid and got no return on. These are not abstract environmental metrics. They are line items that show up on your settlement statement every cycle [3,4].'),

      h2('2.1  Reducing Feed Waste'),
      para('Feed represents 60 to 70 percent of the total cost of broiler production [4]. That single number explains why feed efficiency is the most watched metric in the industry, and why even small improvements in feed conversion ratio (FCR) show up directly in profitability. A 0.1-point improvement in FCR on a 20,000-bird barn at current feed prices cuts real money off your cost per kilogram.'),
      para('Feed waste comes from several specific places, and most of them are fixable without capital expense:'),
      bullet([{ text: 'Feeder height: ', bold: true }, { text: 'The most common waste point in broiler barns. Feeders set too low allow birds to scratch feed out. Feeders set too high cause birds to strain and spill. Adjust feeder height to the shoulder level of the birds weekly throughout the grow-out [3,11].' }]),
      ...
      bullet([{ text: 'Phase feeding: ', bold: true }, { text: 'Feeding a diet suited to the growth stage the flock is actually in stops you from overfeeding expensive amino acids and phosphorus later in the grow-out. Most integrators provide phase-feeding schedules; follow them [4].' }]),
      bullet([{ text: 'Feed storage integrity: ', bold: true }, { text: 'Water getting into bins causes caking, mold, and mycotoxin build-up that cuts intake and hurts bird performance. Check bin seals and aeration systems between cycles.' }]),
      ...
      ...image(photoBuf('Photo2.2-feeder correct height.png'), 'Photo 2.1: Feeder height comparison. Too low (left): feed spills over the rim onto the litter floor, one of the most common and most correctable feed waste points in a broiler barn. Shoulder level (right): no spill, no waste. Feeder height must be checked and adjusted weekly as birds grow [3,11]. Source: CPC Short Courses.'),

      para('There is a less obvious kind of feed waste that never shows up as spilled pellets: feed the bird eats but converts poorly. When birds are under heat stress, or when the feed itself has gone rancid or moldy, the damage that builds up inside the bird, called oxidative stress, hurts the gut and the immune system, and feed conversion drifts the wrong way. Every point the FCR slips means more feed, and all the land, water, and energy behind it, burned to grow the same kilogram of meat. Keeping birds cool and feeding fresh, well-stored feed is a feed-efficiency measure as much as a welfare one. For how oxidative stress works inside the bird, see Course 6 (Poultry Anatomy and Physiology) in this series.'),

      h2('2.2  Water-Saving Practices'),
      para('Water efficiency in a poultry barn is mostly about drinker management and leak detection. Nipple drinker systems are more water-efficient than bell drinkers when maintained correctly, but a nipple system with poor pressure regulation or worn seals can leak just as badly as an open-water system [6].'),
      bullet([{ text: 'Line pressure: ', bold: true }, { text: 'The CPC Learning Centre Managing the Nipple Drinker Line bulletin explains that low-pressure systems must start at the day-one setting and have both pressure and line height increased as birds grow. Never run a low-pressure system at high pressure. It puts water on the floor, not into the birds. In hot weather, raise pressure slightly so birds can drink enough. Adjust pressure throughout the grow-out and check the litter under drinker lines daily as your guide [12].' }]),
      bullet([{ text: 'Water meters: ', bold: true }, { text: 'Install a meter on every barn and read it at the same time every day, at minimum during the first week of placement and during hot weather. A sudden increase in daily consumption may signal a leak in the drinker system, while a sudden decrease can indicate a health problem in the flock, often before mortality numbers rise [6].' }]),
      bullet([{ text: 'Line flushing and cleaning: ', bold: true }, { text: 'Biofilm builds inside drinker lines over the course of a flock cycle. Salmonella and other pathogens can survive a simple water flush inside that biofilm. The CPC Learning Centre Drinking Water Management guide recommends cleaning lines between every flock with an approved acid or hydrogen peroxide product to break down biofilm fully before new birds are placed [6,13].' }]),
      ...
      para('Heating and ventilation are your two biggest energy costs in a broiler barn, and both have real room to improve without major spending [10].'),
      bullet([{ text: 'Brooder management: ', bold: true }, { text: 'Pre-heat the barn and get floor temperature to target before chicks arrive. Chilling them down on day one sets back their immune system and their starter performance for the whole grow-out [3].' }]),
      bullet([{ text: 'Minimum ventilation programs: ', bold: true }, { text: 'In cold weather, minimum ventilation is not optional. It removes moisture that condenses in litter and insulation, protects air quality for the birds, and prevents ammonia accumulation. Cutting ventilation to save heat almost always costs more in wet litter management and performance losses than it saves in gas [10].' }]),
      bullet([{ text: 'Lighting: ', bold: true }, { text: 'LED lighting uses 75 to 85 percent less energy than incandescent bulbs for the same light output, and poultry-grade LED systems are dimmable and long-lasting. The CPC Learning Centre Lighting Program Guidelines for Broilers recommends LED over incandescent and CFL (Compact Fluorescent) systems specifically for their energy efficiency, dimming compatibility, and long service life. Payback on a full barn conversion has been documented in Canadian operations in future [14,15,16].' }]),
      bullet([{ text: 'Barn insulation: ', bold: true }, { text: 'Insulation gaps and failing vapor barriers are common in older barns. A thermal camera scan between flocks shows you exactly where heat is escaping. Fixing those spots costs a lot less than burning extra gas all winter to compensate.' }]),
      bullet([{ text: 'Variable-speed fans: ', bold: true }, { text: 'Fixed-speed fans run at full power or not at all. Variable-speed controllers let the fan match the demand. Over a full year the savings add up, especially in the shoulder seasons when you do not need full ventilation.' }]),
      ...image(figBuf('fig2_3.png'), 'Figure 2.1: LED lighting energy savings compared to incandescent and CFL alternatives. LED systems use 75 to 85 percent less energy than incandescent bulbs for the same light output, support dimmable light management programs, and have documented payback periods in future in Canadian broiler barn conversions [14,15].'),
      ...image(figBuf('fig2_1.png'), 'Figure 2.2: Resource efficiency in a commercial broiler barn. For each of the three major inputs (feed, water, energy), the diagram contrasts common waste points against actionable efficiency measures. Every resource saved per kilogram of gain is a direct cost reduction.'),
      pageBreak(),
    ],
  };
}
...
function buildSection3() {
  return {
    ...
    children: [
      ...
      para('Manure is the most visible byproduct of poultry production, and how you manage it determines whether it becomes an asset or a liability. Handled well, poultry litter is one of the best organic fertilizers a crop farmer can get. Handled poorly, it is a source of water contamination, air quality complaints, and regulatory problems [1,8]. The difference between those two outcomes is mostly a matter of planning and timing.'),

      h2('3.1  Turning Manure into Fertilizer'),
      para('A 20,000-bird barn running a 6 to 8-week cycle produces roughly 20 to 33 tonnes of litter at whole-house cleanout, depending on litter depth and moisture [4,8]. Send that litter for analysis and you will typically see 2.5 to 3.5 percent total nitrogen, 2.0 to 3.5 percent phosphorus pentoxide, and 2.0 to 3.5 percent potassium oxide [8]. At current fertilizer prices, that is real money for the crop farmer who receives it, and applied properly over time it builds soil rather than depletes it.'),
      ...
      bullet([{ text: 'Apply at the right time: ', bold: true }, { text: 'Fall application on bare soil before freeze-up loses the most nutrients. Spring pre-plant or working it into a growing crop gives the crop the best chance to actually use what you put on [1].' }]),
      ...
      para('Odor and ammonia from litter storage and land application are the most common reasons neighbours complain about poultry operations in Canada. Getting it right keeps you on the right side of both the regulator and the people who live nearby [9]. A few practical steps make a real difference:'),
      bullet([{ text: 'Cover stored litter: ', bold: true }, { text: 'Open litter piles generate ammonia continuously and provide an ideal environment for flies. Cover piles with tarps or move litter quickly to a covered storage facility. A covered pile has significantly lower odor and nitrogen loss than an exposed pile [8].' }]),
      bullet([{ text: 'Compost before land application: ', bold: true }, { text: 'Composting raw litter kills pathogens, cuts odor during application, and holds nutrients better so less ammonia escapes when you spread it [8]. Well-composted litter is also easier to spread uniformly.' }]),
      bullet([{ text: 'Litter amendments: ', bold: true }, { text: 'Products such as alum (aluminum sulfate) and other acidifying amendments cut ammonia in the barn by lowering litter pH, and lock up phosphorus in applied litter so less runs off. Less ammonia means better air quality, drier litter, and better bird performance [7,9].' }]),
      ...
      bullet([{ text: 'Inform neighbors: ', bold: true }, { text: 'Before a major land application, let your neighbours know ahead of time. It sounds simple, but it prevents most of the complaints that turn into regulatory inspections.' }]),
      ...image(photoBuf('Photo3.2-litter-management.png'), 'Photo 3.1: Litter storage comparison. An uncovered pile (left) volatilizes ammonia, attracts flies, and loses nitrogen through runoff at the base. A tarped pile (right) retains nutrients, controls odor, and is far easier to defend in a neighbor complaint or a regulatory inspection [8]. Source: CPC Short Courses.'),

      h2('3.3  Safe Storage and Handling'),
      para('Litter that is stored correctly is a managed resource. Litter that is left in poor conditions becomes a source of pathogen spread, fly breeding, and nutrient runoff. The basics of safe storage are not complicated, but they need to be built into your routine between flocks [1,8]:'),
      bullet('Store litter away from surface water, drainage tiles, and groundwater wells. Your province sets the minimum setback distance from water bodies; check your provincial nutrient management regulations for the specific requirement in your area.'),
...
function buildSection4() {
  return {
    ...
    children: [
      ...
      para('A sick bird is the most expensive animal on your farm. It eats feed and drinks water but converts neither into what you need. If it requires medication, that is a real cost. If it dies, that is a total loss of every input you put in since placement. Good welfare management and sound production management are not two separate tracks. On a well-run farm, they are the same decisions, made in the same morning walk [11,17]. This section looks at flock health through the sustainability lens: how keeping birds healthy protects the feed, water, and money you have already put into them. The full welfare framework, the on-farm welfare indicators, and how to score and act on each one are covered in Course 13 (Poultry Welfare) in this series.'),

      h2('4.1  Healthy Birds as a Sustainability Pillar'),
      para('Mortality and condemnation rates are the most direct signal of how well the flock is being managed. A bird condemned at the plant was raised to slaughter weight but cannot be sold. That is the worst result you can get from every input you spent raising that bird. In commercial broiler production, total grow-out mortality typically runs 3 to 4 percent; above 4 to 5 percent something in the management chain is not working, whether that is biosecurity, nutrition, water quality, ventilation, or disease pressure [3,17].'),
      ...photo(photoBuf('photo4_1.jpg'), 'Photo 4.1: Day-old broiler chicks arriving at a commercial operation. Chick quality at placement is the starting point for flock health. Good brooder management in the first 72 hours sets the foundation for immune development and starter performance for the entire grow-out [3,17]. Photo: USDA/Joe Valbuena, public domain.', 431, 640, 3.5),
      para('Every treatment you avoid through better management is money saved and one less compliance issue. Canada has been steadily tightening restrictions on antibiotics used in food animals, and that direction is not changing [18]. Farms that have built health into daily management rather than relying on treatment to fix problems are already ahead of where the regulations are heading. The ones that have not started that shift are going to face it under more pressure and with less time to adjust. For more on the diseases that most commonly affect welfare and performance in Canadian commercial broiler flocks, see Course 7 (Common Poultry Diseases) in this series.'),
      para('Good biosecurity is the foundation. It keeps new pathogens out of the barn and prevents cross-contamination between flocks and between neighboring operations. It does not require expensive equipment. It requires consistent habits: dedicated footwear, controlled access, downtime between farm visits, and a dead-bird disposal system that does not pile up disease risk right beside where your next flock will live [17]. For more on biosecurity protocols for Canadian commercial poultry operations, see Course 2 (Biosecurity) in this series.'),
      para('Catching a problem early is as important as keeping it out. The CPC Learning Centre Spotting Disease Early guide recommends entering the barn quietly at the start of each daily walk, letting the birds settle, then looking for any change from the day before: flock distribution, feeding and drinking behavior, droppings consistency, vocalization, and unusual mortality [19]. One change in any of those is enough reason to investigate before the next morning walk.'),

      h2('4.2  Housing and Ventilation'),
      para('Ventilation is the single most important management system in a broiler barn for litter quality, air quality, and bird welfare. The goal of a minimum ventilation program in cold weather is not complicated: move enough air to remove the moisture the birds produce, while keeping the barn warm enough for performance [10]. That balance point is not the same every day. It changes with outside temperature, bird age, stocking density, and litter condition.'),
      para('The target for litter moisture is 20 to 25 percent. Above 30 percent, you start to see the consequences: footpad dermatitis, breast blisters, increased ammonia, and a higher disease burden [7]. Footpad dermatitis in particular is a welfare and commercial quality issue. Severe footpad scoring at the plant reduces carcass value and comes back to you on your grading sheet. By the time you see bad pads at the plant, the litter problem that caused them has already been affecting your birds for weeks. For more on the systematic daily monitoring framework that covers litter condition, air quality, water, temperature, and feed checks in a structured barn walk, see Course 3 (T-FLAWS Assessment Management Tool) in this series.'),
      ...
      bullet('Too many birds for the ventilation system to handle'),
      bullet('Cold, damp weather when the heating cannot keep up'),
      para('What brings it down:'),
      bullet('Increasing minimum ventilation rate (even at the cost of some heat)'),
      bullet('Running the barn a bit warmer so the air can carry more moisture out'),
      bullet('Fixing drinker leaks immediately when discovered'),
      bullet('Using a litter amendment during high-moisture periods to hold ammonia down and keep the litter drier [7]'),
      ...image(figBuf('photo 4.2.png'), 'Photo 4.2: Litter moisture comparison in a commercial broiler barn. Target litter at 20 to 25% moisture (left) is dry and friable, with birds moving freely. Problem litter above 30% moisture (right) is compacted and wet, with birds resting on a crusted surface. Footpad dermatitis, breast blisters, and elevated ammonia follow rapidly. If litter feels wet underfoot at day 14, find the cause immediately [7]. Source: CPC Short Courses.'),
      ...image(photoBuf('photo4-3-aaap_fpd_scoring_2022.png'), 'Photo 4.3: Footpad dermatitis scoring guide (AAAP, 2022). Score 0: normal color, no lesions (pass). Score 1: hyperkeratosis with lesions covering less than half the footpad (pass). Score 2: lesions covering more than half the footpad with hemorrhage or swelling (fail). Score 2 at the plant signals a litter management failure 2 to 3 weeks prior in the grow-out [7,17]. Source: American Association of Avian Pathologists, 2022.'),

      h2('4.3  Reducing Flock Stress'),
      para('Birds under constant stress have weaker immune systems and get sick more easily. Acute events, catching, feed withdrawal, temperature spikes, and sudden lighting changes, can cause a jump in mortality and a lasting performance drag that never fully recovers [17]. Most stress in a commercial flock comes from management decisions, and most of it is preventable.'),
      bullet([{ text: 'Stocking density: ', bold: true }, { text: 'Follow integrator guidelines. Overcrowding limits air circulation, worsens litter quality, increases competition for feed and water, and raises the aggression level in the barn. The short-term gain from placing extra birds is rarely worth the management problems it creates [3,17].' }]),
      bullet([{ text: 'Light management: ', bold: true }, { text: 'Use a slow dimming at the end of the day so birds can settle calmly and you see less panicking and piling. Give them real dark time to rest; that supports growth and welfare. And avoid sudden light changes, especially during catching, since that is where panic injuries happen that hurt carcass quality at the plant [3].' }]),
      ...
      bullet([{ text: 'Temperature management: ', bold: true }, { text: 'Birds at the wrong temperature will show you immediately: panting and wing spreading mean heat stress, huddling near the brooders means cold. Either way, energy that should be going into growth is going into staying comfortable instead, and prolonged thermal stress pushes up mortality [3,10].' }]),
      pageBreak(),
    ],
  };
}
...
function buildSection5() {
  return {
    ...
    children: [
      ...
      para('The biggest gains in sustainability on most farms come from changing how you manage things, not from buying new equipment. The management changes cost nothing or close to it, and most of them improve production performance at the same time. The modest investments, LED lighting, fan speed controllers, and litter amendments, pay back quickly [14,15]. The larger capital options, solar and biogas, take longer but have become genuinely accessible for Canadian farms as equipment costs have fallen and federal grant programs have expanded [20,21].'),

      h2('5.1  Composting and Reuse of Materials'),
      para('Composting is one of the most practical sustainability tools available to poultry farmers, and it works for both litter and dead birds [8]. Done right, it kills pathogens, cuts odor, reduces the volume of material you need to move, and turns what would otherwise be a disposal problem into a fertilizer product that is easier to spread and more consistent in its effect on the crop than raw litter.'),
      h3('In-Barn Mortality Composting'),
      para('A covered mortality composting bin in a dedicated area beside the barn can manage routine day-to-day mortalities on the farm. Each layer should be covered with dry litter or another carbon source, the pile should be monitored to ensure active composting temperatures are reached, and the material should be moved at the right stage so the process finishes properly. When well managed, this approach can reduce labor, improve daily mortality handling, and help control odor, although farms facing higher disease pressure or stricter biosecurity demands may prefer an incinerator for faster carcass disposal [22].'),
      h3('Between-Flock Windrow Composting'),
      para('If you clean out between flocks, windrow composting the removed litter on a pad or in a covered area turns it into a more stable fertilizer with far fewer pathogens before it goes on the land. It also shrinks the volume you need to haul, which cuts transport costs [8].'),
      h3('Partial Litter Reuse'),
      para('In-place litter management, where litter is managed across multiple flocks rather than fully removed each time, is common in broiler production and can reduce heating costs and keep microbial communities in the litter that naturally work against Salmonella. The key condition is that litter quality must stay within the target moisture and ammonia range. Partial litter that is wet or heavily caked needs to be removed even if the full cleanout is not scheduled. Caked litter in corners and under feeders and drinkers provides no benefit and a real Salmonella risk [7].'),

      h2('5.2  Natural Light and Airflow'),
      para('In an existing barn, your options for natural light and passive airflow are mostly set by how the barn was built. That said, there are a few retrofit options worth knowing:'),
      bullet([{ text: 'Translucent ridge panels: ', bold: true }, { text: 'Where the barn design allows, translucent polycarbonate or fiberglass panels at the ridge or on the south roof face can cut how much artificial light you need during the long summer days. The trade-off is heat gain through those panels in hot weather, so factor that in before installing.' }]),
      bullet([{ text: 'Natural ventilation supplementation: ', bold: true }, { text: 'In mild weather, you can sometimes let the barn breathe a bit more on its own by cracking the inlets and taking some load off the fans, as long as the barn was built and insulated for that and you are keeping a close eye on temperature, humidity and litter so things do not get too damp or too dusty.' }]),
      para('In Canada\'s climate, you still cannot count on natural air alone for commercial broilers, so good mechanical ventilation is needed year round to control heat, moisture and ammonia and to keep birds meeting welfare and performance targets [17].'),
      ...
      bullet('Put in an application to the federal Agricultural Clean Technology Program for cost-share help, and check what your province offers: in BC, BC Hydro\'s self-generation program is the relevant starting point for on-farm solar [23]; in Ontario, the Agricultural Stewardship Initiative covers similar energy upgrades [21].'),
      bullet('Get more than one quote and work with installers who can show real experience on farm and barn projects, not just houses or city rooftops.'),
      h3('Biogas (Anaerobic Digestion)'),
      para('Anaerobic digestion is a process that takes organic material such as poultry litter and carcasses, breaks it down without oxygen and turns it into biogas you can use for heat or electricity, plus a leftover liquid residue you can spread on fields as fertilizer [8,24]. The technology itself works and there are Canadian farms running digesters, but the money side is trickier than solar, because digesters cost a lot to build and only pay off when you can use or sell the energy and other byproducts in a steady, reliable way.'),
      ...photo(photoBuf('photo5_1.jpg'), 'Photo 5.1: Solar photovoltaic panels installed on a farm building roof. South-facing barn roofs with adequate structural load capacity are well-suited for commercial solar installations. Under provincial net metering programs, surplus electricity generated on the farm is credited against the hydro bill, and the federal Agricultural Clean Technology Program covers 40% of eligible project costs for most applicants [20,21]. Photo: Robin Stott / geograph.org.uk, CC BY-SA 2.0.', 640, 427),
      pageBreak(),
    ],
  };
}
...
function buildSection6() {
  return {
    ...
    children: [
      ...
      para('The financial case for sustainable management is not complicated. Better feed efficiency means lower cost per kilogram of gain. Fewer drinker leaks mean better litter and fewer amendments to buy. Tight energy management means a lower heating bill every cycle. These are not environmental bonuses that might show up someday on a balance sheet. They show up in this cycle, on your settlement statement [24].'),

      h2('6.1  Lowering Production Costs'),
      para('The most direct financial payoff from running a tighter operation is spending less per kilogram of gain. Feed efficiency and energy cost are where most of that saving shows up.'),
      bullet([{ text: 'Feed efficiency: ', bold: true }, { text: 'A 0.1-point improvement in FCR on a 20,000-bird barn over a grow-out cycle represents a direct reduction in feed cost per kilogram of meat produced. At current feed prices, this is a meaningful number per cycle, and it compounds across every cycle you run [4].' }]),
      bullet([{ text: 'Water management: ', bold: true }, { text: 'Undetected drinker leaks waste water, damage litter, and cost more in litter amendments, ventilation adjustments, and performance losses than the water itself costs. Daily water metering is the lowest-cost monitoring tool available and often pays for itself in a single cycle [6].' }]),
      bullet([{ text: 'Litter management: ', bold: true }, { text: 'Good litter quality reduces the volume of litter amendments needed, reduces the cost of between-flock cleanout when it is required, and reduces the disease pressure that leads to medication costs [7].' }]),
      bullet([{ text: 'Energy: ', bold: true }, { text: 'Better insulation, LED lights, and variable-speed fan controls can help take some of the bite out of your power bill every flock. Canadian poultry sources show LEDs can save real money over time, but how fast they pay for themselves depends on what you\'re starting with, how many hours the lights run, your hydro rate, and whether there are any rebates on the table [14,15,25].' }]),
      bullet([{ text: 'Manure value: ', bold: true }, { text: 'Litter sold or land-applied with proper nutrient documentation has real financial value to crop operations. Well-managed, analyzed litter commands better pricing and longer-term supply relationships than litter of unknown or poor quality [8].' }]),

      h2('6.2  Market Value and Consumer Trust'),
      para('The demand for chicken is not going away, but what buyers and retailers expect from the farms behind it is changing. Organic and premium segments have grown steadily, and the retailers running those programs are pushing their requirements further down the supply chain [26]. Most consumers never see a farm. They trust the certification or the label their store puts in front of them, and those labels are increasingly tied to documented farm-level compliance.'),
      para('This matters to you because your integrator is already being required by its retail customers to show that contracted farms meet defined environmental and welfare standards. Farms with documented records, a clean compliance history, low medication use, and participation in animal care programs are in a stronger position when contract renewals come around [17,18].'),
      para('Third-party audited chicken programs in Canada, including Chicken Farmers of Canada\'s Raised by a Canadian Farmer Animal Care and On-Farm Food Safety programs, require farms to meet documented standards for bird care, feed and water management, biosecurity, cleanliness, flock monitoring, and record keeping. If you are already running a tight, well-documented operation, the standards may feel familiar, but certification still means proving that work through the required records and audits [27].'),
      ...
      bullet([{ text: 'Input cost volatility: ', bold: true }, { text: 'Feed prices, energy costs, and water rates all move around. Farms that already use less per kilogram of gain feel those price spikes less [4].' }]),
      ...
      bullet([{ text: 'Antimicrobial resistance: ', bold: true }, { text: 'The rules on antibiotics in food animals get a little tighter every year. Farms that have already cut their dependence through better management will handle the next tightening better than farms that have not started [18].' }]),
...
function buildSection7() {
  return {
    ...
    children: [
      ...
      para('An example of a very basic action plan at the farm may be the following:'),
      numbered('Pick one area to focus on this flock, such as feed, water, energy use, litter moisture, or bird health.'),
      numbered('Choose one concrete improvement in that area that you can actually measure, like tightening feeders, fixing leaking pipes, or adjusting a fan speed.'),
...
function buildSummarySection() {
  return {
    ...
    children: [
      ...
      para('Sustainability in poultry farming is not a separate program running alongside your production operation. It is the production operation, managed well. Every practice in this course that cuts waste, tightens efficiency, or keeps you ahead of regulations also makes the farm more profitable and more durable.'),
      para('The key points to carry forward from this course:'),
      bullet([{ text: 'Environmental impact is manageable: ', bold: true }, { text: 'The water, soil, and air impacts of poultry production are real, but they are manageable with standard good-management practices. Nutrient management planning, covered manure storage, proper litter moisture control, and biosecurity are not special sustainability add-ons. They are the foundation of a well-run farm [1,2,8].' }]),
      bullet([{ text: 'Resource efficiency = profit: ', bold: true }, { text: 'Feed waste, water leaks, and energy loss all have a direct financial cost that is paid every cycle. Closing those gaps is both the right environmental decision and the highest-return management investment available [4,6].' }]),
      bullet([{ text: 'Litter is an asset: ', bold: true }, { text: 'Poultry litter has real fertilizer value. Managed properly, it replaces synthetic fertilizer costs for the farms that receive it and builds long-term soil health. Managed poorly, it becomes a source of regulatory and community problems [8].' }]),
      bullet([{ text: 'Animal welfare and production efficiency align: ', bold: true }, { text: 'Birds that are uncomfortable, overcrowded, or poorly ventilated do not perform well. Managing housing, ventilation, stocking density, and access to feed and water for their welfare is the same thing as managing for better FCR and lower mortality [7,17].' }]),
      bullet([{ text: 'Renewable energy is accessible now: ', bold: true }, { text: 'Solar PV is operating on Canadian poultry farms today, with net metering programs in place across provinces and federal and provincial programs available to support on-farm energy investments. The time to evaluate feasibility is before energy prices rise further, not after [20,21].' }]),
      bullet([{ text: 'Sustainability protects your market position: ', bold: true }, { text: 'Integrators, retailers, and consumers are moving toward documented environmental and welfare standards. Farms with good records and documented practices are better positioned for the contract and market environment that is already here [24].' }]),
...
function buildReferencesSection() {
  return {
    ...
    children: [
      ...
      numberedRef('Miles DM, Rowe DE, Cathcart TC. High litter moisture content suppresses litter ammonia volatilization. Poult Sci. 2011;90(7):1397-1405. doi:10.3382/ps.2010-01114.'),
      ...
      numberedRef('Xin H, Berry IL, Tabler GT, Costello TA. Heat and moisture production of poultry and their housing systems: broilers. Trans ASAE. 2001;44(6):1851-1857.'),
      ...
      numberedRef('Tabler T, Farnell M, Wells J. LED Bulbs: Much to Offer the Poultry Industry [Publication P2894]. Starkville: Mississippi State University Extension Service; 2019.'),
      ...
      numberedRef('Canadian Academy of Health Sciences (CAHS). Antimicrobial Resistance / Antimicrobial Use in Food-Producing Animals in Canada: Strategic Interventions to Strengthen Antimicrobial Stewardship. Ottawa: Canadian Academy of Health Sciences; 2025.'),
      ...
      numberedRef('Clarke S, Ward D. Lighting for poultry with light-emitting diodes (LEDs) [Factsheet]. Ontario Ministry of Agriculture, Food and Rural Affairs; 2017. ISSN 1198-712X. Available from: https://www.ontario.ca/page/lighting-poultry-light-emitting-diodes-leds'),
