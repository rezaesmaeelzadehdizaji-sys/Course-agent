...
function buildIntroSection() {
  return {
    ...
    children: [
      ...
      bullet('Talk about what sustainability really means on a poultry farm, basically keeping the place running strong for years to come without wearing out your land, birds or wallet.'),
      bullet('Point out practical ways to farm smarter, like cutting down waste, saving on input costs and not beating up the environment while you are at it.'),
      bullet('Lay out how raising chickens affects the soil, water, air and the folks living nearby, both the good and the not so good.'),
      bullet('Make better use of what you have, including feed, water, power and buildings, so nothing is being wasted or overused.'),
      bullet('Keep your soil in good shape and cut down on pollution by handling manure the right way instead of letting it pile up into a problem.'),
      bullet('Run your flock in a way that keeps birds healthy and calm, while avoiding losses that could have been prevented with better management.'),
      bullet('Take a hard look at options like solar panels or biogas and figure out if they make sense for your farm and your budget.'),
      bullet('Step back and look at your own operation honestly, then pick a few real, doable changes that will make things better going forward.'),
      bullet('Articulate the financial benefits of sustainable management: lower costs, better performance, and better market position.'),
    ],
  };
}
...
function buildSection5() {
  return {
    ...
    children: [
      ...
      bullet('In mild weather, you can sometimes let the barn breathe a bit more on its own by cracking the inlets and taking some load off the fans, as long as the barn was built and insulated for that and you are keeping a close eye on temperature, humidity and litter so things do not get too damp or too dusty.'),
...
function buildSection7() {
  return {
    ...
    children: [
      ...
      para('A simple action plan on the farm could look like this:'),
      numbered('Pick one area to work on for this flock, such as feed, water, energy, litter, or bird health.'),
      numbered('Pick one clear thing you want to change in that area, something you can point to and measure, like tightening up feeders, fixing leaks, or tweaking a fan setting.'),
      numbered('Before you touch anything, write down where you are starting from, such as your current feed conversion, how much water the barn uses in a day, your power bill for a flock, or what the litter moisture usually runs.'),
      numbered('Put the change in place and keep writing down the same numbers so you can see if it helped, did nothing, or made things worse, then decide your next step for the next flock.'),
