...
function buildIntroSection() {
  return {
    ...
    children: [
      ...
      new Paragraph({ text: 'Introduction', heading: HeadingLevel.HEADING_1, pageBreakBefore: true, spacing: { before: 360, after: 160 } }),
...
function buildSection1() {
  return {
    ...
    children: [
      ...
      para('For most Canadian poultry farmers, your everyday audit is your national on-farm assurance program, delivered through your commodity board. For chicken, that means the Chicken Farmers of Canada Raised by a Canadian Farmer On-Farm Food Safety Program [1] and the Raised by a Canadian Farmer Animal Care Program [2]. Both are mandatory for registered farmers, and both rest on a science-based foundation. The Animal Care Program is built on the poultry Code of Practice developed by the National Farm Animal Care Council [3], the agreed national standard for how poultry should be cared for and handled. The On-Farm Food Safety Program is built on HACCP, the internationally recognized Hazard Analysis Critical Control Point system. Between them, these programs turn the national standards into things an auditor can actually check.'),
      ...
      bullet([{ text: 'Regulatory inspections: ', bold: true }, { text: 'These come from government rather than your board. The Canadian Food Inspection Agency sets the voluntary National Avian On-Farm Biosecurity Standard [7], and can inspect a farm directly under federal animal-health and food-safety law, especially during a disease outbreak, a trace-back, or a food safety recall. Provincial agencies may inspect for environmental rules, deadstock disposal, or labor safety.' }]),
      ...
      ...photoImage(figBuf('fig1.jpg'), 'Figure 1.1: The layers of oversight on a Canadian poultry farm. The rules are set nationally, delivered and audited by your provincial board, with the federal regulator and your buyers adding their own checks. Source: CPC Short Courses.', 5.9),
      para('The audit also protects you directly. It is the proof behind your product. When a grocery chain or an export market asks how Canadian farmers can guarantee safe, humanely raised birds, the answer is the audited on-farm program every registered farmer follows. And it is not just farmers checking themselves. The national program is itself audited every year by an accredited third party, so the whole system gets an outside check, not just your barn [2]. Without it, every farm would have to prove itself on its own, and one bad operator could drag down the price everyone gets.'),
...
function buildSection2() {
  return {
    ...
    children: [
      ...
      ...photoImage(figBuf('fig2.jpg'), 'Figure 2.1: The six areas every poultry auditor checks. Each one combines something the auditor sees in the barn with something they read in your records. Source: CPC Short Courses.', 5.9),
      ...
      para('They will also look for the things that let disease walk in: gaps under doors, feed spills drawing in wild birds, standing water, rodent bait stations that are empty or missing, and bird-proofing on air inlets. It all comes down to two things: keeping disease off the farm, and stopping it from spreading if it does get in. The Canadian Food Inspection Agency National Avian On-Farm Biosecurity Standard lays out how to do both, built around managing who and what gets access, keeping the flock healthy, and running clean day-to-day operations [7]. An auditor is really just checking whether your daily habits deliver those two things. Because biosecurity is the backbone of the whole audit, it is worth knowing in depth. For the full set of protocols, from line-of-separation design to disinfectant selection and PPE, see Course 2 (Biosecurity) in this series.'),
      ...
      ...image(figBuf('early_disease_detection_flock.png'), 'Photo 2.1: The daily walk and the daily record go together. A clipboard in the barn means what you see on the walk gets logged on the spot, from mortality to bird condition. Source: CPC Short Courses.'),

      h2('2.6 Safety and Compliance'),
      para('The last area is about running a safe, compliant operation for the people and the surroundings, not just the birds. The auditor will look at personal protective equipment being available and used, and at how chemicals are stored. Disinfectants, pesticides, and medications need to be labeled, kept in their original containers, stored away from feed, and kept in date so nothing past its expiry is still on the shelf. They will check how you handle and dispose of deadstock, since improper deadstock disposal is both a biosecurity risk and a regulatory one. Pesticide application, fuel and propane handling, and basic staff training round out this area. Fire safety fits here too. The poultry Code of Practice asks every barn to be ready for an emergency like a fire, so keep a charged, in-date fire extinguisher within easy reach and a simple fire and evacuation plan your whole crew knows [3]. A quick look at the gauge and the service tag now and then keeps it from lapsing. None of it is complicated, but it is easy to let slide, and it is exactly the kind of thing an outside set of eyes will catch.'),
    ],
  };
}
...
function buildSection4() {
  return {
    ...
    children: [
      ...
      ...photoImage(figBuf('Audit day.jpg'), 'Photo 4.1: Audit day starts at the entry. Records pulled and ready, dedicated boots and coveralls set out, the visitor log open, and the same boot change and sign-in the auditor follows to go in with you. Source: CPC Short Courses.', 5.9),
...
function buildReferencesSection() {
  return {
    ...
    children: [
      ...
      numberedRef('Egg Farmers of Canada. Start Clean-Stay Clean on-farm food safety program. Ottawa: Egg Farmers of Canada; [cited 2026 Jul]. Available from: eggfarmers.ca'),
      numberedRef('Turkey Farmers of Canada. On-Farm Food Safety Program and Flock Care Program. Ottawa: Turkey Farmers of Canada; [cited 2026 Jul]. Available from: turkeyfarmersofcanada.ca'),
      numberedRef('Egg Quality Assurance (EQA). Trust in every egg: the EQA certification mark. [cited 2026 Jul]. Available from: eggquality.ca'),
      numberedRef('Canadian Food Inspection Agency. National Avian On-Farm Biosecurity Standard. 2nd ed. Ottawa: CFIA; [cited 2026 Jul]. Available from: inspection.canada.ca'),
