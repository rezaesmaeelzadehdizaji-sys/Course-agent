...
const children = [
  ...
  para('Field service is the link between the farm and everyone who supports it. The service technician visits growers regularly, checks how the flock is progressing, catches problems before they get expensive, and keeps the farmer connected to veterinary, nutritional, and management support. Under Canadian supply management the grower holds the quota, markets the chicken, and is paid a live price per bird, so nothing you recommend happens unless the grower decides it is worth doing. The course also separates the field service representative, who works for a veterinary care or animal health company and covers vaccination, pre-placement preparation, and on-farm technical problems, from the feed company representative, whose job is the production data.'),
