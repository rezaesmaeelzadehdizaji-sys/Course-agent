...
const OUT_DIR    = path.join(__dirname, 'CPC Farm Visit Checklist');
const OUT_FILE   = path.join(OUT_DIR, 'Poultry_Farm_Visit_Checklist.docx');
const LOGO_PATH    = path.join(__dirname, 'logo.png');   // clinic logo + wordmark
const TAGLINE_PATH = path.join(__dirname, 'Abbvet Logo-2.png');   // "together we ARE animal care"

// CPC Short Courses brand colors (sampled from logo)
const MED_BLUE  = '2E74B5';   // clinic blue
const DARK_BLUE = '1F3864';   // deep blue for the title
const GOLD      = 'C9A84C';   // clinic green (accent rules / underlines)
const BODY      = '3C3C3C';
const GRAY      = '888888';
const LABEL_BG  = 'EDF2F9';   // very light blue for label cells
const HDR_BG    = '2E74B5';   // section bar / table header (clinic blue)
...
function buildFooter() {
  return new Footer({ children: [new Paragraph({
    children: [
      new TextRun({ text: 'CPC Short Courses  |  Comprehensive Farm Visit Checklist  |  Page ', color: GRAY, size: 18, font: 'Calibri' }),
...
const taglineBuffer = null;  // CPC version: no clinic tagline
const C = [];

// ---- COVER BLOCK ----
C.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 120, after: 40 },
  children: [new TextRun({ text: 'CPC SHORT COURSES', bold: true, color: MED_BLUE, size: 22, font: 'Calibri' })] }));
if (logoBuffer) {
  // logo.png (CPC logo) is 300 x 300 (square) — keep a 1:1 embed
...
C.push(hint('T-FLAWS is the CPC barn-assessment framework: Temperature, Feed, Light, Air, Water, and Sanitation and Space. Log the readings at bird level, then work through each checkpoint. For the full method, see Course 3 (T-FLAWS Assessment Management Tool) in this series.'));
...
C.push(hint('Walk the flock quietly first, then handle a sample of birds. Record counts out of the number examined so findings stay comparable between visits. For full disease profiles and distinguishing signs, see Course 7 (Common Poultry Diseases) in this series.'));
...
C.push(hint('Record gross findings by system and note every sample taken for the laboratory. For post-mortem technique and normal organ appearance, see Course 10 (Necropsy, Normal Birds) and Course 11 (Necropsy, Common Diseases) in this series.'));
...
C.push(hint('For sample selection, packaging, and lab submission, see Course 9 (The Value of Poultry Diagnostics) in this series.'));
C.push(blankGridTable(
