...
function buildStockingTable() {
  ...
  return new Table({
    ...
    rows: [
      ...
      ...rows.map((row, ri) => new TableRow({ children: row.map((cell, ci) => dataCell(cell, ci, ri % 2 === 1)), cantSplit: true })),
    ],
  });
}
...
function buildSection3(gaitBuf) {
  return {
    ...
    children: [
      ...
      ...photo(gaitBuf, 'Figure 3.1: Bristol gait scoring scale for broiler chickens. The six-point scale is from Kestin et al. 1992 (Vet Rec 131:190-194). Birds scoring GS 3 or above are in pain and require immediate attention. Source: CPC Short Courses.', 5.8),
...
function buildSection4() {
  return {
    ...
    children: [
      ...
      para('Laying hen welfare in Canada is undergoing the most significant transition in the industry\'s history. Egg Farmers of Canada committed in February 2016 to stop installing new conventional housing and move the sector to alternative systems [15]. The NFACC Code of Practice for Pullets and Laying Hens then set the deadlines. Every hen has to be in an enriched cage or a non-cage system that meets the Code by July 1, 2036 [7]. Any hen still in a conventional cage after July 1, 2031 must be given at least 90 square inches of space, and enriched cages installed after January 1, 2032 have to include amenities for foraging and dust bathing [7]. The industry is transitioning to enriched cages and cage-free systems, each with different welfare profiles.'),
      para('The Code was amended in July 2025, and the changes land hardest if you run pullets or a multi-tier barn. NFACC updated round feeder space for pullets and laying hens, raised the minimum space allowance for pullets older than eight weeks in multi-tier systems, and capped how many tiers a laying or rearing facility can have [7]. Work from the amended version, not an older printout.'),
      ...photo(figBuf('cage system.jpg'), 'Photo 4.1: A conventional battery cage (left) next to enriched cage systems (center and right). Compared with the bare battery cage, enriched cages give hens more vertical and horizontal space and private nesting areas (the red curtains, center), plus scratch pads on the floor (right). These features let hens nest, perch, and scratch, which reduces the frustration seen in battery cages. Source: Egg Farmers of Alberta (eggs.ab.ca).', 5.8),
...
function buildSection5() {
  return {
    ...
    children: [
      ...
      para('Thermal management is a welfare intervention at every stage. The CPC Learning Centre Heat Stress Technical Bulletin identifies thermal stress as one of the most significant welfare and production risks in commercial poultry, with effects ranging from reduced feed intake and lower growth rates to immune suppression and increased mortality [16].'),
      para('There is a name for what heat and other chronic stressors do inside the bird: oxidative stress. Under heat, crowding, or poor feed, the bird\'s cells make damaging molecules faster than their natural defenses can clear them, and that damage quietly wears down the gut lining and the immune system before you ever see a sick bird. It is a big part of why thermal stress ends up cutting feed intake and growth and raising mortality [16]. Keeping birds in their comfort zone is one of the most direct welfare and performance tools you have. For how oxidative stress works and how the bird defends against it, see Course 6 (Poultry Anatomy and Physiology) in this series.'),
      ...
      ...photo(figBuf('litter management.jpg'), 'Photo 5.2: Dry versus wet litter. Left: dry, friable litter that crumbles in the hand, which is the target. Right: dark, wet, caked litter that packs into a ball and drives ammonia, footpad lesions, and disease. Source: fresheggsdaily.blog and fidarfeed.com.', 5.8),
...
function buildSection6() {
  return {
    ...
    children: [
      ...
      bullet('Time feed withdrawal so the birds are off feed about 8 to 12 hours total by slaughter, counting catching, transport, and the wait at the plant, not 8 to 12 hours on the farm on top of the journey [21]. Too short leaves full crops and guts that contaminate carcasses at the plant; too long causes weight loss and weakens the gut. Leave water in front of the birds until the catching crew is ready, then pull it just before they start [21]. Follow your integrator\'s specific withdrawal schedule.'),
...
function buildSection7() {
  return {
    ...
    children: [
      ...
      bullet('Refresh welfare training regularly, not just at hire. The NFACC Code requires staff to stay competent and get additional training as needed, and it specifically calls for euthanasia skills to be refreshed periodically [1]. A short annual welfare refresher, covering handling, daily monitoring, euthanasia, and when to call the veterinarian, keeps the whole team sharp and catches bad habits before they cost you. The CPC Short Courses series, this course included, is a ready-made way to run that refresher.'),
...
function buildReferencesSection() {
  return {
    ...
    children: [
      h1('References'),
      numberedRef('National Farm Animal Care Council. Code of Practice for the Care and Handling of Hatching Eggs, Breeders, Chickens and Turkeys. Burnaby, BC: NFACC; 2016 (update initiated 2023; public comment expected 2026, revised Code projected 2027). Available from: nfacc.ca [cited 2026 Jul].'),
      ...
      numberedRef('Farm Animal Welfare Council. Five Freedoms [Press statement]. Surrey, UK: FAWC; 1979. Available from: webarchive.nationalarchives.gov.uk [cited 2026 Jul].'),
      ...
      numberedRef('National Farm Animal Care Council. Code of Practice for the Care and Handling of Pullets and Laying Hens. Burnaby, BC: NFACC; 2017, amendments published 2025 Jul 31. Available from: nfacc.ca [cited 2026 Jul].'),
      ...
      numberedRef('Egg Farmers of Canada. Egg Farmers of Canada announces industry-wide transition away from conventional housing [Press release]. Ottawa: Egg Farmers of Canada; 2016 Feb [cited 2026 Jul]. Available from: eggfarmers.ca'),
...
(async () => {
  ...
  const gaitBuf        = figBuf('fig3.1.jpg');
  ...
  const entriesWithAnchor = [
    ...
    { lvl: 2, text: '1.3 The Five Domains: Where the Science Is Now',                              page: 5  },
    { lvl: 2, text: '1.4 Why Welfare Connects Directly to Production',                             page: 6  },
    { lvl: 1, text: 'Section 2: What Your Birds Need',                                             page: 8  },
    { lvl: 2, text: '2.1 Feed and Water Access',                                                   page: 8  },
    { lvl: 2, text: '2.2 Environment: Temperature, Air Quality, and Litter',                       page: 8  },
    { lvl: 2, text: '2.3 Lighting for Welfare and Production',                                     page: 9  },
    { lvl: 2, text: '2.4 Health Care and Disease Management',                                      page: 9 },
    { lvl: 2, text: '2.5 Space, Movement, and Natural Behavior',                                   page: 9 },
    { lvl: 1, text: 'Section 3: Welfare Indicators Every Farmer Should Know',                      page: 11 },
    { lvl: 2, text: '3.1 Gait and Leg Health in Broilers',                                        page: 11 },
    { lvl: 2, text: '3.2 Footpad Dermatitis',                                                     page: 12 },
    { lvl: 2, text: '3.3 Hock Burns',                                                             page: 12 },
    { lvl: 2, text: '3.4 Keel Bone Condition in Laying Hens',                                     page: 13 },
    { lvl: 2, text: '3.5 Feather Condition and Pecking Injuries',                                  page: 14 },
    { lvl: 2, text: '3.6 Daily Mortality and Flock Behavior',                                     page: 16 },
    ...
    { lvl: 2, text: '4.2 Laying Hen Welfare and Housing Systems',                                  page: 17 },
    { lvl: 2, text: '4.3 Breeder Welfare Considerations',                                          page: 19 },
    { lvl: 1, text: 'Section 5: Reducing Stress and Improving the Environment',                    page: 21 },
    { lvl: 2, text: '5.1 Thermal Comfort at Every Age',                                           page: 21 },
    { lvl: 2, text: '5.2 Ventilation and Air Quality',                                            page: 22 },
    { lvl: 2, text: '5.3 Litter Quality Management',                                              page: 23 },
    { lvl: 2, text: '5.4 Light Programs That Support Welfare',                                    page: 24 },
    { lvl: 1, text: 'Section 6: Handling, Transport, and Slaughter',                               page: 26 },
    { lvl: 2, text: '6.1 Calm Handling on the Farm',                                              page: 26 },
    { lvl: 2, text: '6.2 Catching and Loading for Transport',                                     page: 26 },
    { lvl: 2, text: '6.3 From Farm to Plant: Reducing Pre-Slaughter Stress',                      page: 27 },
    { lvl: 1, text: "Section 7: The Farmer's Daily Welfare Role",                                  page: 30 },
    { lvl: 2, text: '7.1 Daily Welfare Walk-Through',                                             page: 30 },
    { lvl: 2, text: '7.2 Recording and Reporting Problems',                                       page: 30 },
    { lvl: 2, text: '7.3 Building a Welfare-Competent Team',                                      page: 31 },
  ...
  if (!/w:styleId="TOC1"/.test(stylesXml)) {
    const tocStyles =
      '<w:style w:type="paragraph" w:styleId="TOC1"><w:name w:val="toc 1"/><w:pPr><w:tabs><w:tab w:val="right" w:leader="dot" w:pos="8640"/></w:tabs><w:spacing w:after="45"/></w:pPr><w:rPr><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/><w:sz w:val="22"/></w:rPr></w:style>' +
      '<w:style w:type="paragraph" w:styleId="TOC2"><w:name w:val="toc 2"/><w:pPr><w:tabs><w:tab w:val="right" w:leader="dot" w:pos="8640"/></w:tabs><w:spacing w:after="45"/><w:ind w:left="440"/></w:pPr><w:rPr><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/><w:sz w:val="22"/></w:rPr></w:style>';
