...
const GREEN     = 'C9A84C';    // clinic green
const BODY      = '3C3C3C';
const GRAY      = '888888';
...
function buildHeader() {
  return new Header({ children: [new Paragraph({
    children: [ new TextRun({ text: 'CPC Short Courses  |  ', color: GRAY, size: 17, font: 'Calibri' }), new TextRun({ text: 'Rapid Farm Visit Checklist', bold: true, color: MED_BLUE, size: 17, font: 'Calibri' }) ],
    alignment: AlignmentType.RIGHT, border: { bottom: { style: BorderStyle.SINGLE, size: 4, color: GREEN } },
  })] });
}
function buildFooter() {
  return new Footer({ children: [new Paragraph({
    children: [ new TextRun({ text: 'CPC Short Courses  |  Rapid Farm Visit Checklist  |  Page ', color: GRAY, size: 17, font: 'Calibri' }), new TextRun({ children: [PageNumber.CURRENT], color: GRAY, size: 17, font: 'Calibri' }), new TextRun({ text: ' of ', color: GRAY, size: 17, font: 'Calibri' }), new TextRun({ children: [PageNumber.TOTAL_PAGES], color: GRAY, size: 17, font: 'Calibri' }) ],
...
C.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 20, after: 30 }, children: [new TextRun({ text: 'CPC SHORT COURSES', bold: true, color: MED_BLUE, size: 20, font: 'Calibri' })] }));
if (logoBuffer) {
  C.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 }, children: [new ImageRun({ data: logoBuffer, transformation: { width: 68, height: 68 }, type: 'png' })] }));
}
...
C.push(hint('T-FLAWS at bird level: Temperature, Feed, Light, Air, Water, Sanitation and Space. See Course 3 (T-FLAWS Assessment Management Tool) for the full method.'));
C.push(readingsTable([
