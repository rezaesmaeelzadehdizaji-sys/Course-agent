...
function buildSection5() {
  return {
    ...
    children: [
      ...
      para('The main federal law here is the Safe Food for Canadians Act, with its regulations that came fully into force on January 15, 2019 [21]. Together they pulled a patchwork of older food rules into one modern system run by the CFIA. Three ideas sit at the center of it. Licensing means businesses that process, import, or trade food across provincial or national borders need a federal license. Preventive controls mean those businesses must have a written plan to identify and control food safety hazards. Traceability means being able to trace product one step back to where it came from and one step forward to where it went [22].'),
...
function buildSection7() {
  return {
    ...
    children: [
      ...
      para('Provinces also carry their own animal welfare law, and this is where the welfare Codes from Section 3 gain real legal teeth. In British Columbia, the Prevention of Cruelty to Animals Act, the PCA Act, is the main law protecting farm animals from distress [27]. It is enforced by the BC SPCA, whose officers are appointed as special provincial constables for that purpose [28]. Here is the part that matters most for a farmer: under the PCA Act, following the generally accepted NFACC Code of Practice is recognized as a defense against a charge of causing distress. In plain terms, raising your birds to the Code is not just good practice. It is your legal protection.'),
      para('The same multi-board structure covers every poultry type in the province. Alongside the BC Chicken Marketing Board sit BC Egg, the British Columbia Broiler Hatching Egg Commission, and the BC Turkey Marketing Board, each regulating its own sector under the same provincial Act and the same supervisory board [29]. The exact board names and some details differ from province to province, but the shape is the same everywhere: a provincial marketing law, a supervisory body, a commodity board for each poultry type, and a provincial welfare law backing the national Codes. Learn the pattern in one province and you can find your way around any of them.'),
    ],
  };
}
