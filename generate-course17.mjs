...
function buildSection5() {
  return {
    ...
    children: [
      ...
      para('The main federal law here is the Safe Food for Canadians Act, with its regulations that came fully into force on January 15, 2019 [26]. Together they pulled a patchwork of older food rules into one modern system run by the CFIA. Three ideas sit at the center of it. Licensing means businesses that process, import, or trade food across provincial or national borders need a federal license. Preventive controls mean those businesses must have a written plan to identify and control food safety hazards. Traceability means being able to trace product one step back to where it came from and one step forward to where it went [27].'),
      ...
      para('So how does a regulator actually check? For grading, eggs are graded at federally licensed grading stations by CFIA-licensed graders, and the CFIA can suspend a station that does not hold the standard [27]. For claims like organic, free-range, or RWA, the burden of proof sits with the producer and processor. They have to keep the records that back the claim, and the CFIA can ask to see that documentation at any time. Organic carries its own third-party certification on top [27]. And because a false or misleading label is prohibited outright, the CFIA can order a correction, hold the product, or pull a license when a claim does not hold up [26]. The single thread running through all of it is honesty: the label has to match what is actually in the package.'),
    ],
  };
}
...
function buildSection7() {
  return {
    ...
    children: [
      ...
      para('Provinces also carry their own animal welfare law, and this is where the welfare Codes from Section 3 gain real legal teeth. In British Columbia, the Prevention of Cruelty to Animals Act, the PCA Act, is the main law protecting farm animals from distress [32]. It is enforced by the BC SPCA, whose officers are appointed as special provincial constables for that purpose [33]. Here is the part that matters most for a farmer: under the PCA Act, following the generally accepted NFACC Code of Practice is recognized as a defense against a charge of causing distress. In plain terms, raising your birds to the Code is not just good practice. It is your legal protection.'),
      para('The same multi-board structure covers every poultry type in the province. Alongside the BC Chicken Marketing Board sit BC Egg, the British Columbia Broiler Hatching Egg Commission, and the BC Turkey Marketing Board, each regulating its own sector under the same provincial Act and the same supervisory board [34]. The exact board names and some details differ from province to province, but the shape is the same everywhere: a provincial marketing law, a supervisory body, a commodity board for each poultry type, and a provincial welfare law backing the national Codes. Learn the pattern in one province and you can find your way around any of them.'),
    ],
  };
}
