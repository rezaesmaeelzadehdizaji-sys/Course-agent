...
function buildSection1() {
  return {
    ...
    children: [
      ...
      para('Before we get into any single rule, it helps to see the whole map. The three layers are not in competition. Each one handles a different part of the job, and they are designed to fit together so that a chicken raised in British Columbia is held to the same core standard as one raised in Ontario or Nova Scotia.'),
...
function buildSection4() {
  return {
    ...
    children: [
      ...
      para('Plenty of Canadian chicks and poults come from hatcheries in the United States, and imports do not get a lighter touch. They get a heavier one. Bringing day-old chicks or hatching eggs across the border is controlled by the CFIA under the Health of Animals Regulations [15]. The importer has to check the requirements in the CFIA Automated Import Reference System and hold an import permit before anything ships [17]. The load travels with a health certificate signed off on the US side. CFIA import policy requires that certificate to state the source flock is free from communicable disease and has not been exposed to Newcastle disease, fowl typhoid, highly pathogenic avian influenza, pullorum, or ornithosis [18]. The US certificate adds two more conditions: the flock has been in the exporting region for at least 90 days, and commercial flocks take part in a pullorum eradication program Canada recognizes, which in the United States means the National Poultry Improvement Plan [19].'),
      ...
      para('Antimicrobial use is the other animal-health area where the rules and the industry have moved hard. Through the Chicken Farmers of Canada Responsible Antimicrobial Use Strategy, the sector has already eliminated the preventive use of the antibiotics most important to human medicine and continues to push overall use down [20]. To put a name to it, the clearest example is bacitracin, a Category III antibiotic long used in Canadian broiler feed to prevent necrotic enteritis [21]. That is exactly where the effort now sits. Higher-importance drugs like penicillin, a Category II antibiotic [21], are already out of preventive use, gone since 2018, while Category III drugs like bacitracin are the current reduction target [20]. On your farm, this shows up as careful, recorded, veterinarian-guided treatment rather than routine preventive dosing, and as complete medication records with proper withdrawal times. For the full picture on treatment and antimicrobial stewardship, see Course 8 (Fundamentals of Poultry Vaccination and Treatment) in this series.'),
      para('Vaccines have a regulatory side of their own, not just how you give them. In Canada they are treated as veterinary biologics, and the CFIA Canadian Centre for Veterinary Biologics licenses them under the Health of Animals Act [15]. Before a poultry vaccine can be sold or used, the maker has to prove it is pure, safe, potent, and effective for what the label claims, so the products in your vaccination program are federally vetted, the same as your medicines [22]. Avian influenza is the exception that keeps changing: Canada has long controlled it by stamping out infected flocks rather than vaccinating [16], but that approach is now under active review, and where it lands is exactly the kind of fast-moving question Course 18 (Current Poultry Issues) in this series is built to track.'),
...
function buildSection5() {
  return {
    ...
    children: [
      ...
      para('Labeling and grading round out the product standards. Eggs are graded and labeled under federal grading rules, and poultry meat carries inspection and labeling requirements so that what is on the package is accurate. Claims like organic, free-range, or raised without antibiotics (RWA) are regulated too, which means a farmer cannot simply print them. They have to be backed by the practices and the program behind them. So how does a regulator actually check? For grading, eggs are graded at federally licensed grading stations by CFIA-licensed graders, and the CFIA can suspend a station that does not hold the standard [27]. For claims like organic, free-range, or RWA, the burden of proof sits with the producer and processor: they have to keep the records that back the claim, and the CFIA can ask to see that documentation at any time, with organic carrying its own third-party certification on top [27]. And because a false or misleading label is prohibited outright, the CFIA can order a correction, hold the product, or pull a license when a claim does not hold up [26]. The single thread running through all of it is honesty: the label has to match what is actually in the package.'),
    ],
  };
}
