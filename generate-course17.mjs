...
function buildSection5() {
  return {
    ...
    children: [
      ...
      para('So how does a regulator actually check? For grading, eggs are graded at federally licensed grading stations by CFIA-licensed graders, and the CFIA can suspend a station that does not hold the standard [28]. For claims like organic, free-range, or RWA, the burden of proof sits with the producer and processor. They have to keep the records that back the claim, and the CFIA can ask to see that documentation at any time. Organic carries its own third-party certification on top [28]. And because a false or misleading label is prohibited outright, the CFIA can order a correction, hold the product, or pull a license when a claim does not hold up [27]. The single thread running through all of it is honesty: the label has to match what is actually in the package.'),
    ],
  };
}
