...
const OUT_FILE  = path.join(OUT_DIR, 'Regulatory_Framework_in_Poultry_Production.docx');
...
function buildIntroSection() {
  return {
    ...
    children: [
      ...
      para('A note on scope. This course explains the framework, not the fine print of every rule. The CPC team keeps this whole short course series up to date as the rules and programs change. Even so, when a specific number or rule really matters for your farm, it is always best to confirm the current version yourself against a validated source: your provincial board, your processor, or your veterinarian.'),
      ...
      bullet('Understand how supply management works for chicken, turkey, and eggs, and what your quota and the programs mean for the way you run the farm.'),
      bullet('Know what the NFACC Codes of Practice expect for bird care, housing, welfare, transport, and handling, and why following them protects you in law.'),
      bullet('Know what the rules require for biosecurity, bird health, and keeping disease off your farm.'),
      bullet('Understand what falls on you for food safety and processing, from delivering clean birds through humane slaughter, pathogen control, and honest labeling.'),
      bullet('Know which records you have to keep, and what actually happens when an auditor or inspector shows up.'),
...
function buildSection1() {
  return {
    ...
    children: [
      ...
      para('Before we get into any single rule, it helps to see the whole map. Federal, national, provincial: the three layers are not in competition. Each one handles a different part of the job, and they are designed to fit together so that a chicken raised in British Columbia is held to the same core standard as one raised in Ontario or Nova Scotia.'),
      ...
      para('Start with the federal government, because it sets the rules everyone else builds on. Two bodies matter most. Agriculture and Agri-Food Canada sets the policy environment that supply management operates within, and the Farm Products Council of Canada, which oversees the system directly, sits within its portfolio [1]. Alongside it sits the Canadian Food Inspection Agency, usually just called the CFIA. That is the one you deal with on food safety, animal health, and any serious disease that turns up in your barn [2].'),
      ...
      bullet([{ text: 'Agriculture and Agri-Food Canada (AAFC): ', bold: true }, { text: 'the federal farming department. Sets agricultural policy, funds research and risk-management programs, and handles trade [1].' }]),
      ...
      bullet([{ text: 'Farm Products Council of Canada (FPCC): ', bold: true }, { text: 'the federal body that oversees the national supply management system for poultry and eggs. It supervises the four producer agencies that run it and formally approves the quota and levy orders they bring forward, so its role is active oversight, not just watching from the sidelines [3].' }]),
      bullet([{ text: 'National producer agencies: ', bold: true }, { text: 'Chicken Farmers of Canada, Egg Farmers of Canada, Turkey Farmers of Canada, and Canadian Hatching Egg Producers. These set national production levels and run the on-farm food safety and animal care programs.' }]),
...
function buildSection2() {
  return {
    ...
    children: [
      ...
      ...image(figBuf('Slide2.JPG'), 'Figure 2.1: The three pillars that supply management balances. Source: CPC Short Courses.'),
      ...
      para('The third pillar is producer pricing. Prices are set using a cost-of-production approach, so the price is built to cover what it actually costs an efficient farmer to raise the birds. That is the pillar you feel most directly, because it is what lands on your settlement statement.'),
      ...
      para('One more point worth knowing: your quota is a target to hit, not just a ceiling to stay under. Boards give you a small tolerance on either side of it, but miss outside that band in either direction and it costs you. Produce too much and the excess comes off a later allotment, with a levy possible on top. Produce too little and the part of the shortfall you cannot carry forward is simply gone. That two-sided pressure is what keeps everyone’s production matched to the market. Section 7.1 shows exactly how both sides work, and the numbers, in British Columbia.'),
    ],
  };
}
...
function buildSection3() {
  return {
    ...
    children: [
      ...
      para('The foundation for poultry welfare in Canada is the Codes of Practice developed by the National Farm Animal Care Council. These are not vague guidelines. They are detailed, consensus-built national standards written by farmers, veterinarians, researchers, processors, and animal welfare groups working together. For meat birds and breeders, the standard is the Code of Practice for the Care and Handling of Hatching Eggs, Breeders, Chickens, and Turkeys, published in 2016 [6]. That one is being rewritten right now. NFACC has the draft going out for public comment in the fall of 2026, with the new Code expected in 2027, so watch for it [6]. For the laying side, it is the Code of Practice for the Care and Handling of Pullets and Laying Hens, published in 2017 and amended in July 2025 [7]. The amendments changed round feeder space, raised the minimum space for pullets over eight weeks in multi-tier systems, and capped how many tiers a laying or rearing barn can have. If you run layers or pullets, work from the amended version, not an old printout [7].'),
      ...
      para('Transport is its own regulated area, and it changed significantly in recent years. The federal rules for moving animals live in Part XII of the Health of Animals Regulations, and a major update came into force on February 20, 2020, replacing requirements that had not been meaningfully changed since 1977 [11]. The updated rules tightened the maximum times birds can go without feed, water, and rest, and they set clearer expectations for assessing whether an animal is fit to load. The practical message for you is straightforward: birds must be fit for the trip, and the clock on feed, water, and rest starts before they leave your farm. Catching crews and transporters share responsibility, but it begins with the birds you present for loading. For more on judging whether a bird is fit to load and on protecting welfare through catching, handling, and transport, see Course 13 (Poultry Welfare) in this series.'),
    ],
  };
}
...
function buildSection4() {
  return {
    ...
    children: [
      ...
      para('Figure 4.1 walks through the whole sequence, from the first warning signs to the CFIA response and the federal compensation paid for animals ordered destroyed [16]. Get your veterinarian on the phone as soon as something looks wrong. Diagnosis is their job, and their read on the flock is what tells you whether you are looking at a routine problem or a reportable one. Lean on that knowledge. If it is reportable, they make sure the call to the CFIA goes in right away instead of you waiting to see how the week goes. Reporting early costs you nothing if you turn out to be wrong, and it protects every farm around you if you are right. For the clinical picture behind these diseases and how to recognize them, see Course 7 (Common Poultry Diseases) in this series.'),

      h2('4.3 Regulatory Requirements for Hatcheries, Breeders, and Supply Flocks'),
      para('Disease control does not start at the broiler barn. It starts up the chain, with breeders and hatcheries, because a problem there can spread to every flock they supply. Hatcheries in Canada operate under federal oversight, and breeder and hatching-egg flocks run their own food safety and animal care programs through Canadian Hatching Egg Producers. The principle is simple: the cleaner and healthier the top of the pyramid, the safer every flock below it. Careful handling and hygiene at the breeder and hatching-egg stage protects bird health all the way down the chain.'),
      para('Plenty of Canadian chicks and poults come from hatcheries in the United States, and imports do not get a lighter touch. They get a heavier one. Bringing day-old chicks or hatching eggs across the border is controlled by the CFIA under the Health of Animals Regulations [15]. The importer has to check the requirements in the CFIA Automated Import Reference System and hold an import permit before anything ships [17]. The load travels with a health certificate signed off on the US side. CFIA import policy requires that certificate to state the source flock is free from communicable disease and has not been exposed to Newcastle disease, fowl typhoid, highly pathogenic avian influenza, pullorum, or ornithosis [18]. The US certificate adds two more conditions. The flock has to have been in the exporting region for at least 90 days, and commercial flocks have to take part in a pullorum eradication program Canada recognizes. In the United States that means the National Poultry Improvement Plan [19].'),
      para('When avian influenza turns up, that border tightens fast. The certificate has to state that the birds did not come from a control zone set up around a US outbreak of avian influenza or Newcastle disease, and had no contact with birds from one [19]. In plain terms, a hatchery sitting inside a zone cannot ship to you until that zone is lifted. There is a practical reason to know this. If your placement runs late or gets canceled during a US outbreak, this is usually why.'),
      para('Antimicrobial use is the other animal-health area where the rules and the industry have moved hard. Through the Chicken Farmers of Canada Responsible Antimicrobial Use Strategy, the sector has already eliminated the preventive use of the antibiotics most important to human medicine and continues to push overall use down [20]. Higher-importance drugs like penicillin, a Category II antibiotic, went out of preventive use back in 2018 [21]. The current target is Category III, and the clearest example there is bacitracin, long used in Canadian broiler feed to prevent necrotic enteritis [20,21]. On your farm, this shows up as careful, recorded, veterinarian-guided treatment rather than routine preventive dosing, and as complete medication records with proper withdrawal times. For the full picture on treatment and antimicrobial stewardship, see Course 8 (Fundamentals of Poultry Vaccination and Treatment) in this series.'),
      para('Vaccines have a regulatory side of their own, not just how you give them. In Canada they are treated as veterinary biologics, and the CFIA Canadian Centre for Veterinary Biologics licenses them under the Health of Animals Act [15]. Before a poultry vaccine can be sold or used, the maker has to prove it is pure, safe, potent, and effective for what the label claims, so the products in your vaccination program are federally vetted, the same as your medicines [22]. Avian influenza is the exception that keeps changing. Canada has long controlled it by stamping out infected flocks rather than vaccinating [16]. That approach is now under active review, and where it lands is exactly the kind of fast-moving question Course 18 (Current Poultry Issues) in this series is built to track.'),

      h2('4.4 Who Oversees Your Veterinarian'),
      para('You lean on your veterinarian for treatment decisions, so it helps to know that the veterinarian is a regulated professional in their own right. Getting there takes two steps, and they work the same way as the rest of this course: a national standard, then provincial delivery and enforcement.'),
      para('The national step comes first. The National Examining Board, a committee of the Canadian Veterinary Medical Association, runs the qualifying examinations a veterinarian has to pass and issues the Certificate of Qualification that says they meet the national standard for a new graduate [23]. That certificate is the gate, not the license. The Board itself has no power to license anyone or to discipline them [23].'),
      para('The provincial step is the one with teeth. Veterinarians are licensed and held to a standard of practice by the veterinary college in their own province, such as the College of Veterinarians of British Columbia. Setting the requirements and granting the license sit with that college alone, and it is the body a veterinarian answers to for how they practice [24]. So if you ever have a real concern about the care your flock is getting, the provincial college is where that goes.'),
      para('The change that matters most to you came on the drug side, and it came from Health Canada. Since December 1, 2018, every medically important antibiotic used in animals has needed a veterinary prescription, so you can no longer buy these products over the counter [25]. The same change cleaned up what is printed on the product. Growth promotion claims came off those labels, along with the directions for using the drug that way, so a label today supports treating or preventing disease and nothing else. The labels also carry a responsible-use statement and a Pr designation on the front panel, which is the quickest way to tell at a glance that a product is prescription-only [25].'),
      para('To write that prescription, your veterinarian needs a genuine working relationship with your farm, one built on real knowledge of your flock and how you manage it. From there the responsibility is theirs: pick the right drug for the problem, set the withdrawal time so no residues reach the meat or eggs you ship, and keep antibiotics for real need rather than preventive dosing. Sometimes a drug has to be used in a way its label does not cover. That is called extra-label use. Your veterinarian can turn to CgFARAD, the Canadian global Food Animal Residue Avoidance Databank, for a science-based withdrawal interval, so your birds and eggs still clear residue limits [26]. That professional judgment is the backstop behind the industry antimicrobial strategy above, and it is why your medication records and your veterinarian\'s direction always go together.'),
    ],
  };
}
...
function buildSection5() {
  return {
    ...
    children: [
      ...
      para('The main federal law here is the Safe Food for Canadians Act, with its regulations that came fully into force on January 15, 2019 [27]. Together they pulled a patchwork of older food rules into one modern system run by the CFIA. Three ideas sit at the center of it. Licensing means businesses that process, import, or trade food across provincial or national borders need a federal license. Preventive controls mean those businesses must have a written plan to identify and control food safety hazards. Traceability means being able to trace product one step back to where it came from and one step forward to where it went [28].'),
      ...
      para('Labeling and grading round out the product standards. Eggs are graded and labeled under federal grading rules, and poultry meat carries inspection and labeling requirements so that what is on the package is accurate. Claims like organic, free-range, free-run, or raised without antibiotics (RWA) are regulated too, which means a farmer cannot simply print them. They have to be backed by the practices and the program behind them.'),
      para('So how does a regulator actually check? For grading, eggs are graded at federally licensed grading stations by CFIA-licensed graders, and the CFIA can suspend a station that does not hold the standard [28]. For claims like organic, free-range, free-run, or RWA, the burden of proof sits with the producer and processor. They have to keep the records that back the claim, and the CFIA can ask to see that documentation at any time [28].'),
      para('It is worth knowing what each of those claims actually commits you to, because they are not the same job. Organic is the heaviest lift. It runs under the Canada Organic Regime in Part 13 of the Safe Food for Canadians Regulations, and it takes third-party certification, certified organic feed, no antibiotics, and outdoor access for the birds [29]. It is also not a one-time hurdle. You write an organic plan setting out how the operation runs, you keep it current, and a certification body inspects the farm every year to keep the certificate alive [30]. Organic is a standing commitment, not just a harder door to get through.'),
      para('Raised without antibiotics is narrower. There is no special feed to buy, but the birds cannot have received an antibiotic by any route, from egg injection at the hatchery through to the plant [31]. Vaccines are fine, and so are chemical coccidiostats such as amprolium and decoquinate. Ionophores are not. They count as antibiotics for this purpose and they will cost you the claim [31].'),
      para('One point to be clear about. None of this means leaving a sick flock untreated. If your birds need antibiotics, you treat them, and that call belongs with your veterinarian. What you lose is the claim on those birds, not the right to look after them [31].'),
      para('A false or misleading label is prohibited outright, so the CFIA can order a correction, hold the product, or pull a license when a claim does not hold up [27]. The single thread running through all of it is honesty: the label has to match what is actually in the package.'),
    ],
  };
}
...
function buildSection6() {
  return {
    ...
    children: [
      ...
      para('The records you are required to keep are, almost entirely, the same records you should keep to run the place well. That is the key insight: compliance paperwork and good management are the same paperwork. Your on-farm food safety and animal care programs spell out the full list, but it comes down to a handful of things kept current and on file, shown in Figure 6.1 [13].'),
      ...image(figBuf('Slide4.JPG'), 'Figure 6.1: The core records every poultry farm keeps, grouped by type. Source: CPC Short Courses.'),
      para('The habit that makes records easy is writing them down the same day, not reconstructing a month from memory. As the CPC Learning Centre Spotting Disease Early guide stresses, the record only earns its keep when it is filled in while the day is still fresh [14].'),
...
// ---- Provincial small-lot / quota threshold table (Section 7.1) ----
function provincialThresholdTable() {
  const colW = [1900, 3370, 3370]; // twips, sum = 8640
  ...
  const headers = ['Poultry sector', 'British Columbia', 'Ontario'];
  const rows = [
    ['Chicken (broilers)', 'Up to 2,000 birds per year (Small Lot Permit Program)', 'Up to 300 birds per year (Family Food); 600 to 3,000 under the Artisanal Chicken Program'],
    ['Table eggs (laying hens)', 'Up to 399 hens: 0 to 99 backyard, 100 to 399 registered small lot', 'Up to 99 hens'],
    ['Turkeys', 'Up to 300 birds per year (Direct Vendor); up to 50 for personal use', 'Up to 50 birds per year; larger volumes through the Small Scale Grower Program'],
  ];
...
function buildSection7() {
  return {
    ...
    children: [
      ...
      para('One detail worth pinning down is exactly where the line sits between a small lot you can run without quota and a commercial operation that needs it. That line is a bird count, and it differs by province and by sector. As of August 2026, here is the most you can raise before you need quota in British Columbia and in Ontario, side by side [35]:'),
      provincialThresholdTable(),
      new Paragraph({
        children: [new TextRun({ text: 'Table 7.1: The most you can raise without quota in BC and Ontario, by poultry sector. Source: BC and Ontario poultry marketing boards.', italics: true, color: '555555', size: 20, font: 'Calibri' })],
        alignment: AlignmentType.CENTER, spacing: { before: 80, after: 160 },
      }),
      para('These numbers are set by each provincial board and can change, so confirm the current figure with your board before you rely on it. The moment you cross a threshold, you move out of the small-lot or personal-use world and into full quota and the on-farm programs that come with it. The exact limits differ from province to province, but every province draws the same kind of line.'),

      h2('7.2 The Role of Provincial Acts, Marketing Boards, and Welfare Laws'),
      para('Provinces also carry their own animal welfare law, and this is where the welfare Codes from Section 3 gain real legal teeth. In British Columbia, the Prevention of Cruelty to Animals Act, the PCA Act, is the main law protecting farm animals from distress [36]. It is enforced by the BC SPCA, the British Columbia Society for the Prevention of Cruelty to Animals, whose officers are appointed as special provincial constables for that purpose [37]. Here is the part that matters most for a farmer: under the PCA Act, following the generally accepted NFACC Code of Practice is recognized as a defense against a charge of causing distress. In plain terms, raising your birds to the Code is not just good practice. It is your legal protection.'),
      para('The same multi-board structure covers every poultry type in the province. Alongside the BC Chicken Marketing Board sit BC Egg, the British Columbia Broiler Hatching Egg Commission, and the BC Turkey Marketing Board, each regulating its own sector under the same provincial Act and the same supervisory board [38]. The exact board names and some details differ from province to province, but the shape is the same everywhere: a provincial marketing law, a supervisory body, a commodity board for each poultry type, and a provincial welfare law backing the national Codes. Learn the pattern in one province and you can find your way around any of them.'),
    ],
  };
}
...
function buildReferencesSection() {
  return {
    ...
    children: [
      ...
      numberedRef('Agriculture and Agri-Food Canada. About Agriculture and Agri-Food Canada: mandate and role. Ottawa: Government of Canada; [cited 2026 Aug]. Available from: agriculture.canada.ca'),
      numberedRef('Canadian Food Inspection Agency. About the CFIA: mandate, food safety, and animal health. Ottawa: Government of Canada; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('Farm Products Council of Canada. Who we are and what we do. Ottawa: Government of Canada; [cited 2026 Aug]. Available from: canada.ca/en/farm-products-council'),
      numberedRef('Library of Parliament. Canada’s Supply Management System (Background paper, publication 2018-42-E). Ottawa: Library of Parliament; [cited 2026 Aug]. Available from: lop.parl.ca'),
      numberedRef('Farm Products Agencies Act (RSC 1985, c. F-4). Ottawa: Justice Laws Website, Government of Canada; [cited 2026 Aug]. Available from: laws-lois.justice.gc.ca'),
      numberedRef('National Farm Animal Care Council. Code of Practice for the Care and Handling of Hatching Eggs, Breeders, Chickens, and Turkeys. Ottawa: NFACC; 2016 (update initiated 2023; public comment expected 2026, revised Code projected 2027) [cited 2026 Aug]. Available from: nfacc.ca'),
      numberedRef('National Farm Animal Care Council. Code of Practice for the Care and Handling of Pullets and Laying Hens. Ottawa: NFACC; 2017, amendments published 2025 Jul 31 [cited 2026 Aug]. Available from: nfacc.ca'),
      numberedRef('Chicken Farmers of Canada. Raised by a Canadian Farmer Animal Care Program. Ottawa: Chicken Farmers of Canada; [cited 2026 Aug]. Available from: chickenfarmers.ca'),
      numberedRef('Egg Farmers of Canada. Start Clean-Stay Clean and Animal Care Program. Ottawa: Egg Farmers of Canada; [cited 2026 Aug]. Available from: eggfarmers.ca'),
      numberedRef('Turkey Farmers of Canada. Flock Care Program and On-Farm Food Safety Program. Ottawa: Turkey Farmers of Canada; [cited 2026 Aug]. Available from: turkeyfarmersofcanada.ca'),
      numberedRef('Canadian Food Inspection Agency. Health of Animals Regulations Part XII: Transport of Animals. Ottawa: CFIA; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('Canadian Food Inspection Agency. National Avian On-Farm Biosecurity Standard. Ottawa: CFIA; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('Chicken Farmers of Canada. Raised by a Canadian Farmer On-Farm Food Safety Program. Ottawa: Chicken Farmers of Canada; [cited 2026 Aug]. Available from: chickenfarmers.ca'),
      numberedRef('Spotting Disease Early [Flock Management Guide]. CPC Learning Centre. Available from: cpclearningcentre.ca'),
      numberedRef('Health of Animals Act (SC 1990, c. 21). Ottawa: Justice Laws Website, Government of Canada; [cited 2026 Aug]. Available from: laws-lois.justice.gc.ca'),
      numberedRef('Canadian Food Inspection Agency. Fact sheet: avian influenza (reportable disease). Ottawa: CFIA; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('Canadian Food Inspection Agency. Applying to import live animals, semen, embryos, animal products and by-products (permit application for live animals, hatching eggs and animal germplasm; check the Automated Import Reference System for import requirements). Ottawa: CFIA; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('Canadian Food Inspection Agency. Import Reference Document (As referenced in the Health of Animals Regulations), Section 12: hatching eggs. Ottawa: CFIA; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('United States Department of Agriculture, Animal and Plant Health Inspection Service. Export of Day-Old Poultry and Hatching Eggs to Canada [veterinary health certificate]. Riverdale, MD: USDA APHIS; [cited 2026 Aug]. Available from: aphis.usda.gov'),
      numberedRef('Chicken Farmers of Canada. Responsible Antimicrobial Use Strategy. Ottawa: Chicken Farmers of Canada; [cited 2026 Aug]. Available from: chickenfarmers.ca'),
      numberedRef('Diarra MS, Malouin F. Antibiotics in Canadian poultry productions and anticipated alternatives. Front Microbiol. 2014;5:282. doi:10.3389/fmicb.2014.00282'),
      numberedRef('Canadian Food Inspection Agency, Canadian Centre for Veterinary Biologics. The Regulation of Veterinary Biologics in Canada: Overview (guideline 4.10E). Ottawa: CFIA; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('Canadian Veterinary Medical Association, National Examining Board. About the NEB and the Certificate of Qualification (CQ). Ottawa: CVMA; [cited 2026 Aug]. Available from: canadianveterinarians.net'),
      numberedRef('Canadian Veterinary Medical Association. Provincial and territorial veterinary regulatory bodies; veterinary-client-patient relationship (VCPR). Ottawa: CVMA; [cited 2026 Aug]. Available from: canadianveterinarians.net'),
      numberedRef('Health Canada. Responsible use of medically important antimicrobials in animals (veterinary prescription requirement effective 2018 Dec 1). Ottawa: Government of Canada; [cited 2026 Aug]. Available from: canada.ca/en/health-canada'),
      numberedRef('Canadian global Food Animal Residue Avoidance Databank (CgFARAD). Withdrawal interval recommendations for extra-label drug use in food animals. CgFARAD; [cited 2026 Aug]. Available from: cgfarad.ca'),
      numberedRef('Safe Food for Canadians Act (SC 2012, c. 24). Ottawa: Justice Laws Website, Government of Canada; [cited 2026 Aug]. Available from: laws-lois.justice.gc.ca'),
      numberedRef('Canadian Food Inspection Agency. Understanding the Safe Food for Canadians Regulations: a handbook for food businesses. Ottawa: CFIA; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('Canadian Food Inspection Agency. Organic claims on food labels; Canada Organic Regime (Part 13, Safe Food for Canadians Regulations). Ottawa: CFIA; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('Canadian Food Inspection Agency. Canada Organic Regime operating manual (certification requirements, the organic plan, and annual on-site inspection by the certification body). Ottawa: CFIA; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('Canadian Food Inspection Agency. Method of production claims on food labels (including raised without the use of antibiotics). Ottawa: CFIA; [cited 2026 Aug]. Available from: inspection.canada.ca'),
      numberedRef('Natural Products Marketing (BC) Act (RSBC 1996, c. 330). Victoria, BC: King’s Printer; [cited 2026 Aug]. Available from: bclaws.gov.bc.ca'),
      numberedRef('British Columbia Chicken Marketing Board. Governance and our story; General Orders (over- and under-production rules). Abbotsford, BC: BCCMB; [cited 2026 Aug]. Available from: bcchicken.ca'),
      numberedRef('British Columbia Farm Industry Review Board. Regulated marketing. Victoria, BC: Government of British Columbia; [cited 2026 Aug]. Available from: www2.gov.bc.ca'),
      numberedRef('British Columbia Chicken Marketing Board. Small Lot Permit Program; BC Egg. Small lot and backyard flock guidelines; British Columbia Turkey Marketing Board. Direct Vendor (small lot) program; Chicken Farmers of Ontario. Family Food and Artisanal Chicken programs; Egg Farmers of Ontario. Small flock (99-hen) exemption; Turkey Farmers of Ontario. Small Scale Grower Program. BC and Ontario; [cited 2026 Aug]. Available from: bcchicken.ca; bcegg.com; bcturkey.com; ontariochicken.ca; eggfarmersofontario.ca; turkeyfarmers.on.ca'),
      numberedRef('Prevention of Cruelty to Animals Act (RSBC 1996, c. 372). Victoria, BC: King’s Printer; [cited 2026 Aug]. Available from: bclaws.gov.bc.ca'),
      numberedRef('BC SPCA. What is the Prevention of Cruelty to Animals Act (PCA Act)? Vancouver, BC: BC SPCA; [cited 2026 Aug]. Available from: spca.bc.ca'),
      numberedRef('BC Egg Marketing Board; British Columbia Broiler Hatching Egg Commission; BC Turkey Marketing Board. Victoria and Abbotsford, BC; [cited 2026 Aug]. Available from: bcegg.com; bcbhec.com; bcturkey.com'),
    ],
  };
}
...
async function main() {
  ...
  const entriesWithAnchor = [
    ...
    { lvl: 1, text: 'Section 1: Overview of the Regulatory Landscape', page: 5 },
    { lvl: 2, text: '1.1 Who Regulates Poultry in Canada: Federal, Provincial, and Industry', page: 5 },
    { lvl: 2, text: '1.2 Key Regulatory Agencies and Organizations', page: 6 },
    { lvl: 1, text: 'Section 2: Supply Management and Market Regulation', page: 7 },
    { lvl: 2, text: '2.1 How Supply Management Works for Poultry and Eggs', page: 7 },
    { lvl: 2, text: '2.2 Relevant Legislation and Organizational Oversight', page: 8 },
    { lvl: 1, text: 'Section 3: Animal Welfare and Care Standards', page: 9 },
    { lvl: 2, text: '3.1 Codes of Practice for the Care and Handling of Poultry', page: 9 },
    { lvl: 2, text: '3.2 Mandated Housing, Environment, Feeding, Transport, and Handling Standards', page: 9 },
    { lvl: 1, text: 'Section 4: Biosecurity, Animal Health, and Disease Prevention', page: 11 },
    { lvl: 2, text: '4.1 On-Farm Biosecurity Standards and Disease-Control Regulations', page: 11 },
