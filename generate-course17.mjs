...
function buildSection5() {
  return {
    ...
    children: [
      ...
      para('Labeling and grading round out the product standards. Eggs are graded and labeled under federal grading rules, and poultry meat carries inspection and labeling requirements so that what is on the package is accurate. Claims like organic, free-range, or raised without antibiotics (RWA) are regulated too, which means a farmer cannot simply print them. They have to be backed by the practices and the program behind them. So how does a regulator actually check? For grading, eggs are graded at CFIA-registered stations by licensed graders, and the CFIA can suspend a station that does not hold the standard [24]. For claims like organic, free-range, or RWA, the burden of proof sits with the producer and processor: they have to keep the records that back the claim, and the CFIA can ask to see that documentation at any time, with organic carrying its own third-party certification on top [24]. And because a false or misleading label is prohibited outright, the CFIA can order a correction, hold the product, or pull a licence when a claim does not hold up [23]. The single thread running through all of it is honesty: the label has to match what is actually in the package.'),
    ],
  };
}
