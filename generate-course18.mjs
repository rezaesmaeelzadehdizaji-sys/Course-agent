...
function buildSection2() {
  return {
    ...
    children: [
      ...
      para('The classic warning sign of HPAI is sudden, severe death loss, birds dying quickly and in numbers that do not add up. Before or alongside that, the CFIA lists what to watch for: birds gone quiet and deeply depressed, a drop in egg production with soft-shelled or shell-less eggs, diarrhea, swelling of the skin under the eyes, combs and wattles swollen and congested, and hemorrhages on the hocks [1]. Many of these signs overlap with other diseases, which is exactly why you do not try to diagnose it yourself. For how avian influenza signs compare with other common poultry diseases, see Course 7 (Common Poultry Diseases) in this series.'),
...
function buildSection3() {
  return {
    ...
    children: [
      ...
      para('DMV/1639 is also not the end of the story for infectious bronchitis. This virus is a shape-shifter that swaps pieces of its genetic code with other strains, and Canadian diagnostic labs keep turning up brand-new versions. As recently as 2024, two previously unrecognized IBV variants were found in Canadian flocks hit by drops in egg production, each one a recombinant that had blended parts of other viruses into something new [21]. The worry with any new variant is always the same. The vaccine that protected your birds against last year’s strain may not fully cover this year’s, which is why a strain your program handled five years ago is worth rechecking with your veterinarian today.'),
      para('A different disease has been climbing in Canadian broiler barns: inclusion body hepatitis, or IBH. It is caused by fowl adenoviruses, and it usually hits broilers between three and five weeks old as a sudden jump in deaths, with pale, swollen, mottled livers showing up at post-mortem [22]. What marks it as a genuine emerging disease is the trend. IBH outbreaks have been rising in Canada and worldwide since about 2015, and the mix of virus types behind them has shifted. FAdV8b is now the strain found most often, where FAdV8a and FAdV11 used to share the load [23]. IBH has long been the kind of disease that moves in after something else has weakened the birds, usually infectious bursal disease or chicken anemia virus, though it now turns up in flocks as a primary problem on its own [22]. Either way, it is a reminder to look past the liver and ask what else is going on in that flock.'),
      ...
      para('If you want the clearest recent example of a disease that simply was not here and now is, it is avian metapneumovirus, or aMPV. It turned up in Canadian commercial poultry for the first time in 2024, confirmed in two Ontario turkey flocks that April, and it was in Manitoba and Quebec before the year was out [24,25]. It is no longer news. It is established. Ontario alone had recorded over a hundred confirmed cases by late 2025, and the people who work on it now describe it the way they describe infectious bronchitis: a respiratory virus that is going to keep circulating in the industry rather than one we are going to chase back out [25].'),
      para([
        ...
        { text: ' than usual behind it [25]. Vaccines are in limited use in Canada under CFIA-approved doses while full licensing is worked out, so this is a live conversation to have with your veterinarian rather than a settled program [25]. The lesson to take from aMPV is the one this whole section is built on. A disease that was somebody else\u2019s problem in 2023 was in Canadian barns in 2024 and endemic by 2025. That is how fast the map changes.' },
      ]),
      ...
      para('It works like this. Your daily observations, and the birds and samples your veterinarian sends to a provincial diagnostic laboratory, are how new and unusual problems first get spotted. The work that identified the DMV/1639 strain in Canada came out of exactly this kind of laboratory testing of field cases [19]. Those labs feed their results into national systems, including the avian influenza surveillance run through CanNAISS, so that a threat appearing on scattered farms is recognized as a pattern early rather than discovered one barn at a time [16]. The earlier the pattern is seen, the faster vaccines, alerts, and management advice can catch up to it.'),

      h2('3.4 What Farmers Should Do'),
      para('For all the science and surveillance behind it, the most powerful early-warning tool in the country is still a farmer who walks the barn every day and reports what does not look right. You are the first link in the chain, and a problem you catch and report on Monday is far cheaper for everyone than the same problem found across the region a month later [18].'),
...
async function main() {
  ...
  const entriesWithAnchor = [
    ...
    { lvl: 2, text: '2.4 The Outbreak Response and Protecting Your Farm', page: 10 },
    ...
    { lvl: 2, text: '3.3 Surveillance and Early Warning', page: 15 },
    { lvl: 2, text: '3.4 What Farmers Should Do', page: 16 },
