...
function buildSection4() {
  return {
    ...
    children: [
      ...
      para('When your serology report comes back, focus on two numbers: the Mean Titer and the Coefficient of Variation (%CV). The mean titer is the average antibody level across the flock. Your vet or vaccine manufacturer will give you the target range to compare it against. The %CV tells you how evenly the flock responded. A low %CV means birds are all at similar levels and coverage is solid. A high %CV means the flock is split: some birds are well-protected, others barely have any coverage at all. For a full walkthrough of reading serology reports, interpreting %CV thresholds, and acting on the results, see Course 15 (Serology 101) in this series [1].'),

      h2('4.3 Confirming a Field Challenge: Paired Samples'),
      para('A single blood sample showing high titers cannot tell you whether the birds responded well to vaccination or are actively fighting a field virus. To answer that question you need paired samples: an acute sample taken as soon as you notice something is off, and a convalescent sample from the same barn 10 to 14 days later. If titers hold steady, the high level came from the vaccine. If they jump significantly between the two samples, you have confirmed an active field challenge [1].'),
...
async function main() {
  ...
  const entriesWithAnchor = [
    ...
    { lvl: 2, text: '4.2 Interpreting Lab Reports: Mean Titers and Uniformity',                         page: 15 },
    { lvl: 2, text: '4.3 Confirming a Field Challenge: Paired Samples',                                 page: 15 },
