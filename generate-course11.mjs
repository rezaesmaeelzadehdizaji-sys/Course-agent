...
function buildCoverSection() {
  ...
  children.push(
    ...
    new Paragraph({ children: [new TextRun({ text: 'Disclaimer: This course is intended for educational purposes only. The information provided does not replace professional veterinary advice, diagnosis, or treatment. Always consult a licensed veterinarian for flock health decisions.', color: '888888', size: 18, font: 'Calibri', italics: true })], alignment: AlignmentType.JUSTIFIED, spacing: { before: 0, after: 0 } }),
  );
...
function buildBody() {
  ...
  return [
    ...
    para('You are not expected to diagnose Avian Influenza (AI) on the farm. You are expected to recognize when a presentation is suspicious and respond appropriately. The CFIA and your veterinarian take it from there [10].'),
    ...
    para('If you open a bird and see this combination, close the bird, secure the barn, and call your veterinarian immediately. Do not move any birds or equipment off the farm. Highly pathogenic AI is a federally reportable disease in Canada, and the response protocol requires CFIA involvement [10].'),
    ...
    lesionTable(
      ...
      [
        ...
        ['E. maxima', 'Jejunum (mid-gut)', 'Ballooned intestine with orange-pink mucus. Intestinal wall thickened and pink. Lesions often precede necrotic enteritis.'],
    ...
    para('Salpingitis is inflammation of the oviduct, and it is often found alongside egg peritonitis or as a separate cause of production loss and mortality. The oviduct is the first place to examine in any layer or breeder hen that has died or been euthanized due to reproductive failure [15]:'),
    ...
    para('Fowl cholera hits layers and breeders harder than broilers, and it kills fast in the acute form. The CPC Learning Centre Fowl Cholera disease profile describes the acute lesion set clearly [16]:'),
    ...
    para('Acute fowl cholera in a flock with good birds suddenly dead is a production emergency. Culture from fresh liver and heart blood confirms the diagnosis. Pasteurella multocida grows readily on standard culture media within 24 hours [16].'),

    spacer(40),
    para([{ text: 'Mycoplasmosis (MG and MS)', bold: true }]),
    para('Mycoplasma gallisepticum (MG) and Mycoplasma synoviae (MS) are chronic respiratory and synovitis pathogens that cost the layer and breeder industry through reduced production, increased condemnation, and treatment costs rather than through acute mortality [17].'),
    ...
    para('MG and MS are often present together. PCR testing of tracheal or choanal swabs from live birds is the most sensitive diagnostic method, but the gross lesions at necropsy give you strong evidence to work with while you wait for lab results [17].'),
    ...
    para('FLHS kills laying hens at or near peak production, typically those on high-energy diets with limited activity. The necropsy is diagnostic in obvious cases [19]:'),
    ...
    para('The liver dry matter in FLHS birds is at least 40% fat, which explains the friability [19]. These birds are often the heaviest in the flock. Management response: evaluate diet energy density, body weight targets, and whether feed restriction programs are being followed correctly.'),

    spacer(40),
    para([{ text: 'Calcium Deficiency and Cage Layer Fatigue', bold: true }]),
    para('Calcium deficiency in laying hens causes a progressive skeletal failure as bone calcium is mobilized to meet the demand for eggshell formation [20]. At necropsy:'),
    ...
    numberedRef("Merck Veterinary Manual. Marek's Disease in Poultry. Kenilworth, NJ: Merck & Co.; 2024 [cited 2026 Jun]. Available from: merckvetmanual.com/poultry/neoplasms-in-poultry/mareks-disease-in-poultry"),
