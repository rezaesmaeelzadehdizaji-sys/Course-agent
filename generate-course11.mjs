...
function buildBody() {
  ...
  return [
    ...
    para('IBH has become a leading cause of liver disease in Canadian broilers: diagnostic data from 2008 to 2024 show it as the main fowl adenovirus presentation, most often in Ontario, with the FAdV-8b strain dominant since 2015 [15].'),

    spacer(40),
    para([{ text: 'Reovirus (Viral Arthritis / Tenosynovitis)', bold: true }]),
    para('Reovirus causes viral arthritis, also called tenosynovitis. It shows up as lameness in growing broilers, usually from four weeks on. The hock joint and the tendons just above it are the target [16]:'),
    ...
    para('Viral arthritis spreads early in life and gives a swollen-hock picture very similar to Mycoplasma synoviae. Open the joint and check the tendons. Reovirus hits the tendons hardest; MS gives you the creamy joint exudate. The lab sorts out which is which [16].'),

    pageBreak(),
    h2('4.3  Parasitic Conditions'),
    para([{ text: 'Coccidiosis', bold: true }]),
    para('Coccidiosis is one of the most common and costly diseases in floor-raised poultry worldwide, and some level of challenge is present on virtually every broiler farm [17]. The intestinal lesion from coccidiosis depends entirely on which Eimeria species is involved. Each species colonizes a specific segment of the intestine, and the gross appearance varies accordingly:'),
    cocciTable(
      ...
      [
        ...
        ['E. necatrix', 'Jejunum (mid-gut)', 'White and red spots on the serosal surface. Interior shows petechiae and necrotic patches. Mainly affects older birds (over 8 to 9 weeks), so it is more a problem in broiler breeders and layers than in young market broilers [18].', eNec],
        ['E. brunetti', 'Lower small intestine, rectum', 'Hemorrhagic, necrotic mucosa in the lower gut. Watery bloody content. Like E. necatrix, it affects older birds, so it shows up in broiler breeders and layers rather than young market broilers [18].', eBru],
    ...
    para('Internal parasites are less common in modern all-in, all-out broiler production but still appear in multi-age or outdoor-access flocks [19]. At necropsy:'),
    ...
    para('Ascites in broilers is a cardiovascular disease, not an infectious one. Fast-growing birds in cool weather with poor air quality are the highest-risk group. The CPC Learning Centre Ascites disease profile identifies the key lesion sequence [20]:'),
    ...
    para('Sudden death syndrome kills fast-growing, well-conditioned broilers, usually males, with a brief wing-beating convulsion before death. At necropsy the findings are characteristically minimal [21]:'),
    ...
    para('Leg problems are expensive: birds die, the ones that live cannot get to feed and water, and they get downgraded at the plant. Three main conditions show up at necropsy [22]:'),
    ...
    para('A cluster of young chicks with these firm lung nodules points straight back to the hatchery or the litter. Pull a sample of the suspect litter or feed and check your brooding setup for damp, moldy spots [23].'),
    pageBreak(),
    ...
    para('Egg yolk peritonitis is one of the most common causes of odd, one-off deaths in layers and breeder hens around peak lay. The lesions are hard to miss [24]:'),
    ...
    para('Salpingitis is inflammation of the oviduct, and it is often found alongside egg peritonitis or as a separate cause of production loss and mortality. The oviduct is the first place to examine in any layer or breeder hen that has died or been euthanized due to reproductive failure [25]:'),
    ...
    para([
      ...
      { text: ' (MS) are slow, nagging infections rather than fast killers. They rarely cause a big die-off. What they cost you is steady: fewer eggs, more condemnations at the plant, and money spent on treatment [27,28].' },
    ]),
    ...
    para("Marek's is well controlled by hatchery vaccination in Canadian commercial flocks, so when it does appear it usually points to a vaccination failure rather than a new threat [29]. Marek's disease causes two distinct lesion patterns, and knowing which form you are dealing with matters for your vaccination audit:"),
    ...
    para('ILT is a herpesvirus that attacks the windpipe. It hits layers, breeders, and backyard flocks hard, and birds gasp, cough, and shake bloody mucus onto the walls and each other. The necropsy is one of the most distinctive in poultry [30]:'),
    ...
    para('Avian metapneumovirus drives swollen head syndrome in broilers and breeders and a respiratory, drop-in-production picture in layers. It attacks the upper airway first [31]:'),
