...
function buildBody() {
  ...
  return [
    ...
    para('Leg problems in broilers cost the industry through mortality, reduced mobility, and downgrading at the plant. Three main conditions are identifiable at necropsy [21]:'),
    ...
    para('Egg yolk peritonitis is one of the most common causes of sporadic death in layers and breeder hens around peak production. The lesions are difficult to miss [22]:'),
    ...
    para('Egg peritonitis often starts when reverse peristalsis pushes yolk material from the oviduct back into the abdominal cavity, or when a follicle ruptures into the coelom rather than being caught by the infundibulum. Secondary E. coli infection is almost universal once yolk material sits in the warm abdominal cavity [22].'),

    spacer(40),
    para([{ text: 'Salpingitis', bold: true }]),
    para('Salpingitis is inflammation of the oviduct, and it is often found alongside egg peritonitis or as a separate cause of production loss and mortality. The oviduct is the first place to examine in any layer or breeder hen that has died or been euthanized due to reproductive failure [23]:'),
    ...
    para('Fowl cholera hits layers and breeders harder than broilers, and it kills fast in the acute form. The CPC Learning Centre Fowl Cholera disease profile describes the acute lesion set clearly [24]:'),
    ...
    para('Acute fowl cholera in a flock with good birds suddenly dead is a production emergency. Culture from fresh liver and heart blood confirms the diagnosis. Pasteurella multocida grows readily on standard culture media within 24 hours [24].'),
    ...
    para([
      ...
      { text: ' (MS) are chronic respiratory and synovitis pathogens that cost the layer and breeder industry through reduced production, increased condemnation, and treatment costs rather than through acute mortality [25,26].' },
    ]),
    ...
    para('MG and MS are often present together. PCR testing of tracheal or choanal swabs from live birds is the most sensitive diagnostic method, but the gross lesions at necropsy give you strong evidence to work with while you wait for lab results [25,26].'),

    h2('5.3  Viral Diseases'),
    para([{ text: "Marek's Disease", bold: true }]),
    para("Marek's is well controlled by hatchery vaccination in Canadian commercial flocks, so when it does appear it usually points to a vaccination failure rather than a new threat [27]. Marek's disease causes two distinct lesion patterns, and knowing which form you are dealing with matters for your vaccination audit:"),
    ...
    para('ILT is a herpesvirus that attacks the windpipe. It hits layers, breeders, and backyard flocks hard, and birds gasp, cough, and shake bloody mucus onto the walls and each other. The necropsy is one of the most distinctive in poultry [28]:'),
    ...
    para('A bird found dead with blood in the windpipe and no obvious heart or liver disease should put ILT near the top of your list. On histology the lab finds syncytia and intranuclear inclusion bodies in the tracheal lining [28]. ILT spreads on equipment, clothing, and contaminated litter, so a positive flock is also a biosecurity wake-up call.'),
    ...
    para('aMPV rarely kills on its own. The damage comes from the E. coli that follows it through the broken airway lining. A flock with swollen heads and a colibacillosis necropsy picture, especially alongside a production drop, should make you think aMPV underneath it [29].'),

    h2('5.4  Nutritional and Metabolic Issues'),
    para([{ text: 'Fatty Liver Hemorrhagic Syndrome (FLHS)', bold: true }]),
    para('FLHS kills laying hens at or near peak production, typically those on high-energy diets with limited activity. The necropsy is diagnostic in obvious cases [30]:'),
    ...
    para('The liver dry matter in FLHS birds is at least 40% fat, which explains the friability [30]. These birds are often the heaviest in the flock. Management response: evaluate diet energy density, body weight targets, and whether feed restriction programs are being followed correctly.'),

    spacer(40),
    para([{ text: 'Calcium Deficiency and Cage Layer Fatigue', bold: true }]),
    para('Calcium deficiency in laying hens causes a progressive skeletal failure as bone calcium is mobilized to meet the demand for eggshell formation [31]. At necropsy:'),
    ...
    para('Duck viral enteritis, also called duck plague, is a herpesvirus that hits all kinds of waterfowl and kills fast. The necropsy is dramatic [32]:'),
    ...
    para('Sudden heavy death in ducks or geese with these hemorrhagic, plaque-covered guts is duck plague until the lab says otherwise. Get your veterinarian involved early so the right samples reach the lab fresh [32].'),

    spacer(40),
    para([{ text: 'Duck Virus Hepatitis (DVH)', bold: true }]),
    para('Duck virus hepatitis is a fast, fatal disease of young ducklings, usually under a few weeks old. Mortality can reach 95% in a fully susceptible group. The clue is in the posture and the liver [33]:'),
    ...
    para('A batch of young ducklings dying within minutes, arched backward, with spotted-hemorrhage livers, is the classic DVH picture. Rapid diagnosis drives the response, so call your veterinarian fast [33].'),

    spacer(40),
    para([{ text: "Derzsy's Disease (Goose Parvovirus)", bold: true }]),
    para("Derzsy's disease is a parvovirus of goslings and Muscovy ducklings, usually two to four weeks old, named after the Hungarian virologist who first described it. At necropsy [34]:"),
    ...
    para("A cluster of young goslings with rounded pale hearts, ascites, and that penguin stance points at Derzsy's disease. On histology the lab finds intranuclear inclusion bodies in the heart muscle [34]."),

    h2('6.2  Bacterial and Toxic Conditions'),
    para([{ text: 'Riemerellosis (New Duck Disease)', bold: true }]),
    para('Riemerellosis, caused by Riemerella anatipestifer, is one of the most common and costly bacterial diseases of farmed ducks, and it also hits geese and turkeys. It looks a lot like colibacillosis on the table [35]:'),
    ...
    para('Because Riemerellosis and colibacillosis produce almost the same fibrinous picture, culture is the only way to tell them apart. Swab the pericardium, liver, and air sacs for the lab [35].'),

    spacer(40),
    para([{ text: 'Botulism (Limberneck)', bold: true }]),
    para('Botulism is not an infection. It is a poisoning, caused by a toxin from Clostridium botulinum growing in rotting carcasses, maggots, or decaying plant matter that the birds eat. Waterfowl are especially at risk around stagnant water [36].'),
    ...
    para('Here is what makes botulism unique at the necropsy table: there are no lesions. The toxin paralyzes without leaving a mark, so the carcass is usually in good body condition, and you may find maggots in the crop from the spoiled material the bird ate [36]. When you open bird after bird that died with a limber neck and find nothing wrong inside, think botulism and go looking for spoiled feed, a fouled water source, or a dead-carcass pile the flock has been picking at.'),
    pageBreak(),
    ...
    para('Hemorrhagic enteritis is an adenovirus disease of turkeys, usually around six to eleven weeks of age. It causes sudden death, sometimes with bloody droppings [37]:'),
    ...
    para('HE also knocks down the immune system, opening the door to secondary E. coli. A turkey flock with sudden deaths and big mottled spleens fits HE. On histology the lab finds intranuclear inclusion bodies in the spleen [37].'),

    spacer(40),
    para([{ text: 'Blackhead (Histomoniasis)', bold: true }]),
    para('Blackhead, or histomoniasis, is caused by the protozoan Histomonas meleagridis. It is devastating in turkeys, where mortality often runs 80 to 100% [38]. Chickens usually carry it quietly and pass it on. The lesions are so specific they confirm the diagnosis on sight:'),
    ...
    bullet([{ text: 'Liver:', bold: true }, { text: ' Round, sunken, target-like (bullseye) necrotic lesions up to a few centimeters across, green to tan. Together with the cecal cores, these are pathognomonic, meaning they confirm the diagnosis on their own [38].' }]),
    ...
    para('Blackhead has been re-emerging in recent years because there are no longer any approved drugs or vaccines to prevent or treat it, which keeps it a serious threat wherever turkeys are raised or share ground with chickens [38].'),
    pageBreak(),
...
const tocEntries = [
  ...
  { lvl: 2, text: "4.3  Parasitic Conditions", page: 19 },
