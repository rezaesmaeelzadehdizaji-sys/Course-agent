...
function buildBody() {
  ...
  return [
    ...
    spacer(40),
    para([{ text: 'Dehydration and Visceral Gout', bold: true }]),
    para('Dehydration is not a disease in itself. It is what happens when birds cannot get to enough water: a drinker line fails, water pressure or height is set wrong, heat drives demand up, or a gut disease pulls fluid out faster than the bird can replace it. In young chicks it often shows up as first-week starve-out. The necropsy picture is distinctive [28]:'),
    bullet([{ text: 'Eyes and skin:', bold: true }, { text: ' Sunken, dull eyes, and skin that stays tented when you pinch it. The whole carcass often looks dark and dry.' }]),
    ...
    para('Egg yolk peritonitis is one of the most common causes of odd, one-off deaths in layers and breeder hens around peak lay. The lesions are hard to miss [30]:'),
    ...
    para('Egg yolk peritonitis often starts when yolk goes the wrong way, back out of the oviduct into the belly, or when a follicle bursts into the body cavity instead of being caught properly by the oviduct [30].'),

    spacer(40),
    para([{ text: 'Salpingitis', bold: true }]),
    para('Salpingitis is inflammation of the oviduct, and it is often found alongside egg peritonitis or as a separate cause of production loss and mortality. The oviduct is the first place to examine in any layer or breeder hen that has died or been euthanized due to reproductive failure [31]:'),
    ...
    para('Fowl cholera hits layers and breeders harder than broilers, and it kills fast in the acute form. The CPC Learning Centre Fowl Cholera disease profile describes the acute lesion set clearly [32]:'),
    bullet([{ text: 'Acute form:', bold: true }, { text: ' Hemorrhages around the heart and on the lungs. Yellow-white pinpoint spots on the liver surface (multifocal hepatic necrosis). Excess clear fluid in the pericardial sac and abdominal cavity. General reddening of tissues from septicemia. Birds often found dead with no prior illness. In laying hens, the ovary is inflamed with regressing, misshapen follicles (acute oophoritis). In turkeys, one or both lungs are consolidated with fibrinous pleuropneumonia [33].' }]),
    ...
    para('Acute fowl cholera in a flock with good birds suddenly dead is a production emergency. Culture from fresh liver and heart blood confirms the diagnosis. Pasteurella multocida grows readily on standard culture media within 24 hours [32].'),
    ...
    para([
      ...
      { text: ' (MS) are slow, nagging infections rather than fast killers. They rarely cause a big die-off. What they cost you is steady: fewer eggs, more condemnations at the plant, and money spent on treatment [34,35].' },
    ]),
    ...
    para('MG and MS are often present together. PCR testing of tracheal or choanal swabs from live birds is the most sensitive diagnostic method, but the gross lesions at necropsy give you strong evidence to work with while you wait for lab results [34,35].'),

    h2('5.3  Viral Diseases'),
    para([{ text: "Marek's Disease", bold: true }]),
    para("Marek's is well controlled by hatchery vaccination in Canadian commercial flocks, so when it does appear it usually points to a vaccination failure rather than a new threat [36]. Marek's disease causes several distinct lesion patterns, and knowing which form you are dealing with matters for your vaccination audit:"),
    ...
    bullet([{ text: 'Ocular form (gray eye):', bold: true }, { text: ' Lymphocytes infiltrate the iris and fade its normal orange-red color to a flat gray. The pupil turns irregular and off-center, the two eyes are often unequal, and vision fails partly or fully. Seen more in older birds [33].' }]),
    ...
    para('Avian metapneumovirus drives swollen head syndrome in broilers and breeders and a respiratory, drop-in-production picture in layers. It attacks the upper airway first [38]:'),
    ...
    para('aMPV rarely kills on its own. The damage comes from the E. coli that follows it through the broken airway lining. A flock with swollen heads and a colibacillosis necropsy picture, especially alongside a production drop, should make you think aMPV underneath it [38].'),
    spacer(40),
    para([{ text: 'Fowl Pox', bold: true }]),
    para('Fowl pox is a slow-spreading viral disease seen more in layers, breeders, and backyard or free-range birds than in fast-grown broilers. Biting insects, especially mosquitoes, carry it from bird to bird, so outbreaks often track with mosquito season [39]. It shows up in two forms:'),
    ...
    para('Scabby combs and wattles spreading slowly through the flock over weeks, especially after a wet mosquito season, point to fowl pox. The lesions are distinctive on sight, but the lab confirms it [39].'),

    h2('5.4  Nutritional and Metabolic Issues'),
    para([{ text: 'Fatty Liver Hemorrhagic Syndrome (FLHS)', bold: true }]),
    para('FLHS kills laying hens at or near peak production, typically those on high-energy diets with limited activity. The necropsy is diagnostic in obvious cases [40]:'),
    ...
    para('Duck viral enteritis, also called duck plague, is a herpesvirus that hits all kinds of waterfowl and kills fast. The necropsy is dramatic [43]:'),
    ...
    para('Sudden heavy death in ducks or geese with these hemorrhagic, plaque-covered guts is duck plague until the lab says otherwise. Get your veterinarian involved early so the right samples reach the lab fresh [43].'),

    spacer(40),
    para([{ text: 'Duck Virus Hepatitis (DVH)', bold: true }]),
    para('Duck virus hepatitis is a fast, fatal disease of young ducklings, usually under a few weeks old. Mortality can reach 95% in a fully susceptible group. The clue is in the posture and the liver [44]:'),
    ...
    para('A batch of young ducklings dying within minutes, arched backward, with spotted-hemorrhage livers, is the classic DVH picture. Rapid diagnosis drives the response, so call your veterinarian fast [44].'),

    spacer(40),
    para([{ text: "Derzsy's Disease (Goose Parvovirus)", bold: true }]),
    para("Derzsy's disease is a parvovirus of goslings and Muscovy ducklings, usually two to four weeks old, named after the Hungarian virologist who first described it. At necropsy [45]:"),
    ...
    para("A cluster of young goslings with rounded pale hearts, ascites, and that penguin stance points at Derzsy's disease. On histology the lab finds intranuclear inclusion bodies in the heart muscle [45]."),

    h2('6.2  Bacterial and Toxic Conditions'),
    para([{ text: 'Riemerellosis (New Duck Disease)', bold: true }]),
    para('Riemerellosis, caused by Riemerella anatipestifer, is one of the most common and costly bacterial diseases of farmed ducks, and it also hits geese and turkeys. It looks a lot like colibacillosis on the table [46]:'),
    ...
    para('Because Riemerellosis and colibacillosis produce almost the same fibrinous picture, culture is the only way to tell them apart. Swab the pericardium, liver, and air sacs for the lab [46].'),

    spacer(40),
    para([{ text: 'Botulism (Limberneck)', bold: true }]),
    para('Botulism is not an infection. It is a poisoning, caused by a toxin from Clostridium botulinum growing in rotting carcasses, maggots, or decaying plant matter that the birds eat. Waterfowl are especially at risk around stagnant water [47].'),
    ...
    para('Here is what makes botulism unique at the necropsy table: there are no lesions. The toxin paralyzes without leaving a mark, so the carcass is usually in good body condition, and you may find maggots in the crop from the spoiled material the bird ate [47]. When you open bird after bird that died with a limber neck and find nothing wrong inside, think botulism and go looking for spoiled feed, a fouled water source, or a dead-carcass pile the flock has been picking at.'),
    pageBreak(),
    ...
    para('Hemorrhagic enteritis is an adenovirus disease of turkeys, usually around six to twelve weeks of age. It causes sudden death, sometimes with bloody droppings [48]:'),
    ...
    para('HE also knocks down the immune system, opening the door to secondary E. coli. A turkey flock with sudden deaths and big mottled spleens fits HE. On histology the lab finds intranuclear inclusion bodies in the spleen [48].'),

    spacer(40),
    para([{ text: 'Blackhead (Histomoniasis)', bold: true }]),
    para('Blackhead, or histomoniasis, is caused by the protozoan Histomonas meleagridis. It is devastating in turkeys, where mortality often runs 80 to 100% [49]. Chickens usually carry it quietly and pass it on. The lesions are so specific they confirm the diagnosis on sight:'),
    ...
    bullet([{ text: 'Liver:', bold: true }, { text: ' Round, sunken, target-like (bullseye) necrotic lesions up to a few centimeters across, green to tan. Together with the cecal cores, these are pathognomonic, meaning they confirm the diagnosis on their own [49].' }]),
    bullet([{ text: 'Other organs:', bold: true }, { text: ' In severe cases the infection spreads beyond the ceca and liver, and you may find necrotic foci in the pancreas, and less often the kidneys, spleen, or bursa [49].' }]),
    ...
    para('For Canadian context: virulent Newcastle disease had been absent from Canada since 1973, but in June 2025 the CFIA confirmed PPMV-1 in commercial squab pigeon farms in British Columbia\'s Fraser Valley, the first Canadian case in over fifty years [50].'),
    ...
    lesionTable(
      ...
      [
        ...
        ['Liver', 'Firm, dark red-brown, no spots', 'Yellow spots (cholera), friable/pale (FLHS/IBH), green (staph septicemia), fibrin or chalky urate coat', 'Fowl Cholera, FLHS, Colibacillosis, IBH, Staphylococcosis, Gout'],
        ...
        ['Kidneys', 'Dark red, smooth', 'Pale/swollen with chalk-white tubules; ureters packed with urate', 'IBV renal form, IBD, dehydration (visceral gout)'],
        ['Nervous system (peripheral nerves, brain, spinal cord)', 'Nerves uniform white and symmetric; birds alert and coordinated', 'Enlarged, asymmetric peripheral nerves with dull striations (Marek\'s); nervous signs (torticollis, tremors, star-gazing, paralysis) with subtle or absent gross brain lesions', 'Marek\'s Disease (neural), Newcastle Disease (neurotropic), PPMV-1, AI, Fowl Cholera (chronic, torticollis), Botulism'],
        ['Heart/pericardium', 'Clean surface, no excess fluid', 'Fibrin plaques; chalky white urate coat (visceral gout); dilated right heart; contracted ventricles', 'Colibacillosis, Fowl Cholera, Ascites, SDS, AI, Dehydration/Gout'],
        ...
        ['Ovary', 'Orderly cluster of developing yellow follicles', 'Hemorrhagic, regressed, or atretic follicles; ruptured follicles; serofibrinous oophoritis; tumor nodules', 'ND, AI, Marek\'s Disease, Fowl Cholera, Egg Peritonitis, aMPV'],
        ...
        ['Bone (cross-section)', 'Thick, hard cortex; growth plate hardens to bone', 'Thin cortex, fragile, cancellous replacement; crumbly necrotic femoral head; retained white cartilage plug in the growth plate', 'Calcium deficiency, cage layer fatigue, Staphylococcosis, Tibial Dyschondroplasia (TD)'],
    ...
    numberedRef("Merck Veterinary Manual. Urate Deposition (Gout) in Poultry. Kenilworth, NJ: Merck & Co.; 2024 [cited 2026 Jul]. Available from: merckvetmanual.com/poultry/miscellaneous-conditions-of-poultry/urate-deposition-gout-in-poultry"),
...
const tocEntries = [
  ...
  { lvl: 2, text: "5.4  Nutritional and Metabolic Issues", page: 36 },
  { lvl: 1, text: "6. Common Diseases in Ducks and Geese", page: 39 },
  { lvl: 2, text: "6.1  Viral Diseases", page: 39 },
  { lvl: 2, text: "6.2  Bacterial and Toxic Conditions", page: 41 },
  { lvl: 1, text: "7. Common Diseases in Turkeys", page: 43 },
  { lvl: 2, text: "7.1  Hemorrhagic Enteritis and Blackhead", page: 43 },
  { lvl: 1, text: "8. Cross-Species Disease Concerns", page: 45 },
  { lvl: 2, text: "8.1  Pigeon Paramyxovirus (PPMV-1)", page: 45 },
  { lvl: 1, text: "9. Necropsy Lesion Recognition by Body System", page: 46 },
  { lvl: 1, text: "10. Case Studies and Problem-Solving", page: 48 },
  { lvl: 2, text: "10.1  Case 1: Broiler Mortality Spike at 28 Days", page: 48 },
  { lvl: 2, text: "10.2  Case 2: Layer Flock with Mortality and Production Drop", page: 48 },
  { lvl: 2, text: "10.3  Case 3: Sudden Mortality in Young Broilers at 3 Weeks", page: 49 },
