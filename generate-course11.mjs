...
function buildBody() {
  ...
  return [
    ...
    para('IBH is a viral liver disease, caused by a fowl adenovirus, that usually hits broilers between three and seven weeks of age. The CPC Learning Centre Inclusion Body Hepatitis disease profile and the necropsy picture line up well [15,16]:'),
    ...
    para('Fowl pox is a slow-spreading viral disease seen more in layers, breeders, and backyard or free-range birds than in fast-grown broilers. Biting insects, especially mosquitoes, carry it from bird to bird, so outbreaks often track with mosquito season [33]. It shows up in two forms:'),
    ...
    para('Scabby combs and wattles spreading slowly through the flock over weeks, especially after a wet mosquito season, point to fowl pox. The lesions are distinctive on sight, but the lab confirms it [33].'),

    h2('5.4  Nutritional and Metabolic Issues'),
    para([{ text: 'Fatty Liver Hemorrhagic Syndrome (FLHS)', bold: true }]),
    para('FLHS kills laying hens at or near peak production, typically those on high-energy diets with limited activity. The necropsy is diagnostic in obvious cases [34]:'),
    ...
    para('An FLHS liver is at least 40% fat by dry weight, which is why it falls apart so easily [34]. These birds are usually the heaviest in the flock. What to do: look at how rich the feed is, check body weights against target, and make sure any feed-restriction program is actually being followed.'),

    spacer(40),
    para([{ text: 'Calcium Deficiency and Cage Layer Fatigue', bold: true }]),
    para('When a laying hen does not get enough calcium from her feed, her body pulls it straight from her own bones to keep building eggshells. Over time, that constant drain causes osteoporosis and leaves her skeleton weak and brittle. Cage layer fatigue is what you see when that bone loss gets severe enough that the hen can no longer stand up [35]. At necropsy:'),
    ...
    para('Duck viral enteritis, also called duck plague, is a herpesvirus that hits all kinds of waterfowl and kills fast. The necropsy is dramatic [37]:'),
    ...
    para('Sudden heavy death in ducks or geese with these hemorrhagic, plaque-covered guts is duck plague until the lab says otherwise. Get your veterinarian involved early so the right samples reach the lab fresh [37].'),

    spacer(40),
    para([{ text: 'Duck Virus Hepatitis (DVH)', bold: true }]),
    para('Duck virus hepatitis is a fast, fatal disease of young ducklings, usually under a few weeks old. Mortality can reach 95% in a fully susceptible group. The clue is in the posture and the liver [38]:'),
    ...
    para('A batch of young ducklings dying within minutes, arched backward, with spotted-hemorrhage livers, is the classic DVH picture. Rapid diagnosis drives the response, so call your veterinarian fast [38].'),

    spacer(40),
    para([{ text: "Derzsy's Disease (Goose Parvovirus)", bold: true }]),
    para("Derzsy's disease is a parvovirus of goslings and Muscovy ducklings, usually two to four weeks old, named after the Hungarian virologist who first described it. At necropsy [39]:"),
    ...
    para("A cluster of young goslings with rounded pale hearts, ascites, and that penguin stance points at Derzsy's disease. On histology the lab finds intranuclear inclusion bodies in the heart muscle [39]."),

    h2('6.2  Bacterial and Toxic Conditions'),
    para([{ text: 'Riemerellosis (New Duck Disease)', bold: true }]),
    para('Riemerellosis, caused by Riemerella anatipestifer, is one of the most common and costly bacterial diseases of farmed ducks, and it also hits geese and turkeys. It looks a lot like colibacillosis on the table [40]:'),
    ...
    para('Because Riemerellosis and colibacillosis produce almost the same fibrinous picture, culture is the only way to tell them apart. Swab the pericardium, liver, and air sacs for the lab [40].'),

    spacer(40),
    para([{ text: 'Botulism (Limberneck)', bold: true }]),
    para('Botulism is not an infection. It is a poisoning, caused by a toxin from Clostridium botulinum growing in rotting carcasses, maggots, or decaying plant matter that the birds eat. Waterfowl are especially at risk around stagnant water [41].'),
    ...
    para('Here is what makes botulism unique at the necropsy table: there are no lesions. The toxin paralyzes without leaving a mark, so the carcass is usually in good body condition, and you may find maggots in the crop from the spoiled material the bird ate [41]. When you open bird after bird that died with a limber neck and find nothing wrong inside, think botulism and go looking for spoiled feed, a fouled water source, or a dead-carcass pile the flock has been picking at.'),
    pageBreak(),
    ...
    para('Hemorrhagic enteritis is an adenovirus disease of turkeys, usually around six to twelve weeks of age. It causes sudden death, sometimes with bloody droppings [42]:'),
    ...
    para('HE also knocks down the immune system, opening the door to secondary E. coli. A turkey flock with sudden deaths and big mottled spleens fits HE. On histology the lab finds intranuclear inclusion bodies in the spleen [42].'),

    spacer(40),
    para([{ text: 'Blackhead (Histomoniasis)', bold: true }]),
    para('Blackhead, or histomoniasis, is caused by the protozoan Histomonas meleagridis. It is devastating in turkeys, where mortality often runs 80 to 100% [43]. Chickens usually carry it quietly and pass it on. The lesions are so specific they confirm the diagnosis on sight:'),
    ...
    bullet([{ text: 'Liver:', bold: true }, { text: ' Round, sunken, target-like (bullseye) necrotic lesions up to a few centimeters across, green to tan. Together with the cecal cores, these are pathognomonic, meaning they confirm the diagnosis on their own [43].' }]),
    ...
    para('Blackhead has been re-emerging in recent years because there are no longer any approved drugs or vaccines to prevent or treat it, which keeps it a serious threat wherever turkeys are raised or share ground with chickens [43].'),
    pageBreak(),
    ...
    para([{ text: 'Interpretation:', bold: true }, { text: ' Classic acute IBD. The enlarged, hemorrhagic bursa, intramuscular hemorrhages, and renal lesions match the CPC Learning Centre IBD disease profile exactly [10]. The rapid onset, age at peak IBD susceptibility, and high-moisture litter environment support the diagnosis.' }]),
    para([{ text: 'Next steps:', bold: true }, { text: ' Submit fresh bursa from 3 birds to lab for IBD virus isolation and strain identification. Review IBD vaccination timing and maternal antibody data for this breeder flock. Check water quality and litter management. Flag for potential viral variant involvement if variant strains have been circulating in your region [10].' }]),
    pageBreak(),
