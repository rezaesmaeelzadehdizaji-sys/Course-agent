...
function buildBody() {
  ...
  return [
    ...
    para('You are not expected to diagnose Avian Influenza (AI) on the farm. You are expected to recognize when a presentation is suspicious and respond appropriately. The CFIA and your veterinarian take it from there [3,11].'),
    para('Avian influenza is the most pressing poultry disease issue in Canada right now: highly pathogenic H5N1 has been circulating since 2022, with commercial flock outbreaks confirmed across British Columbia, Alberta, Manitoba, Ontario, and Quebec [12].'),
    ...
    para('If you open a bird and see this combination, close the bird, secure the barn, and call your veterinarian immediately. Do not move any birds or equipment off the farm. Highly pathogenic AI is a federally reportable disease in Canada, and the response protocol requires CFIA involvement [3,11].'),

    spacer(40),
    para([{ text: 'Inclusion Body Hepatitis (IBH)', bold: true }]),
    para('IBH is a viral liver disease, caused by a fowl adenovirus, that usually hits broilers between three and seven weeks of age. The CPC Learning Centre Inclusion Body Hepatitis disease profile and the necropsy picture line up well [13,14]:'),
    ...
    para('Reovirus causes viral arthritis, also called tenosynovitis. It shows up as lameness in growing broilers, usually from four weeks on. The hock joint and the tendons just above it are the target [15]:'),
    ...
    para('Viral arthritis spreads early in life and gives a swollen-hock picture very similar to Mycoplasma synoviae. Open the joint and check the tendons. Reovirus hits the tendons hardest; MS gives you the creamy joint exudate. The lab sorts out which is which [15].'),

    pageBreak(),
    h2('4.3  Parasitic Conditions'),
    para([{ text: 'Coccidiosis', bold: true }]),
    para('Coccidiosis is one of the most common and costly diseases in floor-raised poultry worldwide, and some level of challenge is present on virtually every broiler farm [16]. The intestinal lesion from coccidiosis depends entirely on which Eimeria species is involved. Each species colonizes a specific segment of the intestine, and the gross appearance varies accordingly:'),
    cocciTable(
      ...
      [
        ...
        ['E. necatrix', 'Jejunum (mid-gut)', 'White and red spots on the serosal surface. Interior shows petechiae and necrotic patches. Mainly affects older birds (over 8 to 9 weeks), so it is more a problem in broiler breeders and layers than in young market broilers [17].', eNec],
        ['E. brunetti', 'Lower small intestine, rectum', 'Hemorrhagic, necrotic mucosa in the lower gut. Watery bloody content. Like E. necatrix, it affects older birds, so it shows up in broiler breeders and layers rather than young market broilers [17].', eBru],
    ...
    para('Internal parasites are less common in modern all-in, all-out broiler production but still appear in multi-age or outdoor-access flocks [18]. At necropsy:'),
    ...
    para('Ascites in broilers is a cardiovascular disease, not an infectious one. Fast-growing birds in cool weather with poor air quality are the highest-risk group. The CPC Learning Centre Ascites disease profile identifies the key lesion sequence [19]:'),
    ...
    para('Sudden death syndrome kills fast-growing, well-conditioned broilers, usually males, with a brief wing-beating convulsion before death. At necropsy the findings are characteristically minimal [20]:'),
    ...
    para('Leg problems are expensive: birds die, the ones that live cannot get to feed and water, and they get downgraded at the plant. Three main conditions show up at necropsy [21]:'),
    ...
    para('A cluster of young chicks with these firm lung nodules points straight back to the hatchery or the litter. Pull a sample of the suspect litter or feed and check your brooding setup for damp, moldy spots [22].'),
    pageBreak(),
    ...
    para('Egg yolk peritonitis is one of the most common causes of odd, one-off deaths in layers and breeder hens around peak lay. The lesions are hard to miss [23]:'),
    ...
    para('Egg yolk peritonitis often starts when yolk goes the wrong way, back out of the oviduct into the belly, or when a follicle bursts into the body cavity instead of being caught properly by the oviduct. Once yolk is sitting in the warm abdominal cavity, E. coli almost always moves in and infection follows [23].'),

    spacer(40),
    para([{ text: 'Salpingitis', bold: true }]),
    para('Salpingitis is inflammation of the oviduct, and it is often found alongside egg peritonitis or as a separate cause of production loss and mortality. The oviduct is the first place to examine in any layer or breeder hen that has died or been euthanized due to reproductive failure [24]:'),
    ...
    para('Fowl cholera hits layers and breeders harder than broilers, and it kills fast in the acute form. The CPC Learning Centre Fowl Cholera disease profile describes the acute lesion set clearly [25]:'),
    ...
    para('Acute fowl cholera in a flock with good birds suddenly dead is a production emergency. Culture from fresh liver and heart blood confirms the diagnosis. Pasteurella multocida grows readily on standard culture media within 24 hours [25].'),
    ...
    para([
      ...
      { text: ' (MS) are slow, nagging infections rather than fast killers. They rarely cause a big die-off. What they cost you is steady: fewer eggs, more condemnations at the plant, and money spent on treatment [26,27].' },
    ]),
    ...
    para('MG and MS are often present together. PCR testing of tracheal or choanal swabs from live birds is the most sensitive diagnostic method, but the gross lesions at necropsy give you strong evidence to work with while you wait for lab results [26,27].'),

    h2('5.3  Viral Diseases'),
    para([{ text: "Marek's Disease", bold: true }]),
    para("Marek's is well controlled by hatchery vaccination in Canadian commercial flocks, so when it does appear it usually points to a vaccination failure rather than a new threat [28]. Marek's disease causes two distinct lesion patterns, and knowing which form you are dealing with matters for your vaccination audit:"),
    ...
    para('ILT is a herpesvirus that attacks the windpipe. It hits layers, breeders, and backyard flocks hard, and birds gasp, cough, and shake bloody mucus onto the walls and each other. The necropsy is one of the most distinctive in poultry [29]:'),
    ...
    para('Avian metapneumovirus drives swollen head syndrome in broilers and breeders and a respiratory, drop-in-production picture in layers. It attacks the upper airway first [30]:'),
    ...
    para('aMPV rarely kills on its own. The damage comes from the E. coli that follows it through the broken airway lining. A flock with swollen heads and a colibacillosis necropsy picture, especially alongside a production drop, should make you think aMPV underneath it [30].'),

    h2('5.4  Nutritional and Metabolic Issues'),
    para([{ text: 'Fatty Liver Hemorrhagic Syndrome (FLHS)', bold: true }]),
    para('FLHS kills laying hens at or near peak production, typically those on high-energy diets with limited activity. The necropsy is diagnostic in obvious cases [31]:'),
    ...
    para('An FLHS liver is at least 40% fat by dry weight, which is why it falls apart so easily [31]. These birds are usually the heaviest in the flock. What to do: look at how rich the feed is, check body weights against target, and make sure any feed-restriction program is actually being followed.'),

    spacer(40),
    para([{ text: 'Calcium Deficiency and Cage Layer Fatigue', bold: true }]),
    para('When a laying hen does not get enough calcium from her feed, her body pulls it straight from her own bones to keep building eggshells. Over time, that constant drain causes osteoporosis and leaves her skeleton weak and brittle. Cage layer fatigue is what you see when that bone loss gets severe enough that the hen can no longer stand up [32]. At necropsy:'),
    ...
    para('Duck viral enteritis, also called duck plague, is a herpesvirus that hits all kinds of waterfowl and kills fast. The necropsy is dramatic [34]:'),
    ...
    para('Sudden heavy death in ducks or geese with these hemorrhagic, plaque-covered guts is duck plague until the lab says otherwise. Get your veterinarian involved early so the right samples reach the lab fresh [34].'),

    spacer(40),
    para([{ text: 'Duck Virus Hepatitis (DVH)', bold: true }]),
    para('Duck virus hepatitis is a fast, fatal disease of young ducklings, usually under a few weeks old. Mortality can reach 95% in a fully susceptible group. The clue is in the posture and the liver [35]:'),
    ...
    para('A batch of young ducklings dying within minutes, arched backward, with spotted-hemorrhage livers, is the classic DVH picture. Rapid diagnosis drives the response, so call your veterinarian fast [35].'),

    spacer(40),
    para([{ text: "Derzsy's Disease (Goose Parvovirus)", bold: true }]),
    para("Derzsy's disease is a parvovirus of goslings and Muscovy ducklings, usually two to four weeks old, named after the Hungarian virologist who first described it. At necropsy [36]:"),
    ...
    para("A cluster of young goslings with rounded pale hearts, ascites, and that penguin stance points at Derzsy's disease. On histology the lab finds intranuclear inclusion bodies in the heart muscle [36]."),

    h2('6.2  Bacterial and Toxic Conditions'),
    para([{ text: 'Riemerellosis (New Duck Disease)', bold: true }]),
    para('Riemerellosis, caused by Riemerella anatipestifer, is one of the most common and costly bacterial diseases of farmed ducks, and it also hits geese and turkeys. It looks a lot like colibacillosis on the table [37]:'),
    ...
    ...image(photoRiem, 'Photo 6.4: Riemerellosis (new duck disease) lesions. Fibrinous exudate coating the heart (left) and spread over both the liver and heart surfaces (right). Source: Diseases of Poultry, 14th ed.; Merck Veterinary Manual (merckvetmanual.com).', 5.9),
    para('Because Riemerellosis and colibacillosis produce almost the same fibrinous picture, culture is the only way to tell them apart. Swab the pericardium, liver, and air sacs for the lab [37].'),

    spacer(40),
    para([{ text: 'Botulism (Limberneck)', bold: true }]),
    para('Botulism is not an infection. It is a poisoning, caused by a toxin from Clostridium botulinum growing in rotting carcasses, maggots, or decaying plant matter that the birds eat. Waterfowl are especially at risk around stagnant water [38].'),
    ...
    para('Here is what makes botulism unique at the necropsy table: there are no lesions. The toxin paralyzes without leaving a mark, so the carcass is usually in good body condition, and you may find maggots in the crop from the spoiled material the bird ate [38]. When you open bird after bird that died with a limber neck and find nothing wrong inside, think botulism and go looking for spoiled feed, a fouled water source, or a dead-carcass pile the flock has been picking at.'),
    pageBreak(),
    ...
    para('Hemorrhagic enteritis is an adenovirus disease of turkeys, usually around six to eleven weeks of age. It causes sudden death, sometimes with bloody droppings [39]:'),
    ...
    para('HE also knocks down the immune system, opening the door to secondary E. coli. A turkey flock with sudden deaths and big mottled spleens fits HE. On histology the lab finds intranuclear inclusion bodies in the spleen [39].'),

    spacer(40),
    para([{ text: 'Blackhead (Histomoniasis)', bold: true }]),
    para('Blackhead, or histomoniasis, is caused by the protozoan Histomonas meleagridis. It is devastating in turkeys, where mortality often runs 80 to 100% [40]. Chickens usually carry it quietly and pass it on. The lesions are so specific they confirm the diagnosis on sight:'),
    ...
    bullet([{ text: 'Liver:', bold: true }, { text: ' Round, sunken, target-like (bullseye) necrotic lesions up to a few centimeters across, green to tan. Together with the cecal cores, these are pathognomonic, meaning they confirm the diagnosis on their own [40].' }]),
    ...
    para('Blackhead has been re-emerging in recent years because there are no longer any approved drugs or vaccines to prevent or treat it, which keeps it a serious threat wherever turkeys are raised or share ground with chickens [40].'),
    pageBreak(),
    ...
    para('For Canadian context: virulent Newcastle disease had been absent from Canada since 1973, but in June 2025 the CFIA confirmed PPMV-1 in commercial squab pigeon farms in British Columbia\'s Fraser Valley, the first Canadian case in over fifty years [41].'),
...
const tocEntries = [
  ...
  { lvl: 2, text: "10.2  Case 2: Layer Flock with Production Drop and Deaths", page: 46 },
  { lvl: 2, text: "10.3  Case 3: Young Broiler Flock, Sudden Mortality at 3 Weeks", page: 47 },
