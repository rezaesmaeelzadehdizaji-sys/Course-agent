...
function buildBody() {
  ...
  return [
    ...
    para('Necropsying five to six fresh, acutely dead or recently euthanized birds gives you a representative picture of what is happening in the flock [1]. One or two birds is not enough to trust. A single bird can show an incidental finding or a problem that is its own and not the flock\'s. The lesions you can act on are the ones that repeat. So on a larger operation you keep opening birds, often ten to twenty depending on flock size, until a clear, dominant pattern of lesions shows up across the birds you examine. When the same picture keeps appearing bird after bird, that consistent pattern is your real flock-level finding. When the findings stay scattered and inconsistent, keep going, or get your veterinarian and the diagnostic lab involved before you draw any conclusion.'),
    pageBreak(),
