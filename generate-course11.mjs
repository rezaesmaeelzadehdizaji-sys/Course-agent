...
function buildBody() {
  ...
  return [
    ...
    para('Leg problems in broilers cost the industry through mortality, reduced mobility, and downgrading at the plant. Three main conditions are identifiable at necropsy [22]:'),
    ...
    para('Egg yolk peritonitis is one of the most common causes of sporadic death in layers and breeder hens around peak production. The lesions are difficult to miss [23]:'),
    ...
    para([
      ...
      { text: ' (MS) are chronic respiratory and synovitis pathogens that cost the layer and breeder industry through reduced production, increased condemnation, and treatment costs rather than through acute mortality [26,27].' },
    ]),
    ...
    para('A bird found dead with blood in the windpipe and no obvious heart or liver disease should put ILT near the top of your list. On histology the lab finds syncytia and intranuclear inclusion bodies in the tracheal lining [29]. ILT spreads on equipment, clothing, and contaminated litter, so a positive flock is also a biosecurity wake-up call.'),
    ...
    para('The liver dry matter in FLHS birds is at least 40% fat, which explains the friability [31]. These birds are often the heaviest in the flock. Management response: evaluate diet energy density, body weight targets, and whether feed restriction programs are being followed correctly.'),

    spacer(40),
    para([{ text: 'Calcium Deficiency and Cage Layer Fatigue', bold: true }]),
    para('Calcium deficiency in laying hens causes a progressive skeletal failure as bone calcium is mobilized to meet the demand for eggshell formation [32]. At necropsy:'),
    ...
    para('Duck viral enteritis, also called duck plague, is a herpesvirus that hits all kinds of waterfowl and kills fast. The necropsy is dramatic [33]:'),
    ...
    para('Sudden heavy death in ducks or geese with these hemorrhagic, plaque-covered guts is duck plague until the lab says otherwise. Get your veterinarian involved early so the right samples reach the lab fresh [33].'),

    spacer(40),
    para([{ text: 'Duck Virus Hepatitis (DVH)', bold: true }]),
    para('Duck virus hepatitis is a fast, fatal disease of young ducklings, usually under a few weeks old. Mortality can reach 95% in a fully susceptible group. The clue is in the posture and the liver [34]:'),
    ...
    para('A batch of young ducklings dying within minutes, arched backward, with spotted-hemorrhage livers, is the classic DVH picture. Rapid diagnosis drives the response, so call your veterinarian fast [34].'),

    spacer(40),
    para([{ text: "Derzsy's Disease (Goose Parvovirus)", bold: true }]),
    para("Derzsy's disease is a parvovirus of goslings and Muscovy ducklings, usually two to four weeks old, named after the Hungarian virologist who first described it. At necropsy [35]:"),
    ...
    para("A cluster of young goslings with rounded pale hearts, ascites, and that penguin stance points at Derzsy's disease. On histology the lab finds intranuclear inclusion bodies in the heart muscle [35]."),

    h2('6.2  Bacterial and Toxic Conditions'),
    para([{ text: 'Riemerellosis (New Duck Disease)', bold: true }]),
    para('Riemerellosis, caused by Riemerella anatipestifer, is one of the most common and costly bacterial diseases of farmed ducks, and it also hits geese and turkeys. It looks a lot like colibacillosis on the table [36]:'),
    ...
    para('Because Riemerellosis and colibacillosis produce almost the same fibrinous picture, culture is the only way to tell them apart. Swab the pericardium, liver, and air sacs for the lab [36].'),

    spacer(40),
    para([{ text: 'Botulism (Limberneck)', bold: true }]),
    para('Botulism is not an infection. It is a poisoning, caused by a toxin from Clostridium botulinum growing in rotting carcasses, maggots, or decaying plant matter that the birds eat. Waterfowl are especially at risk around stagnant water [37].'),
    ...
    para('Here is what makes botulism unique at the necropsy table: there are no lesions. The toxin paralyzes without leaving a mark, so the carcass is usually in good body condition, and you may find maggots in the crop from the spoiled material the bird ate [37]. When you open bird after bird that died with a limber neck and find nothing wrong inside, think botulism and go looking for spoiled feed, a fouled water source, or a dead-carcass pile the flock has been picking at.'),
    pageBreak(),
    ...
    para('Hemorrhagic enteritis is an adenovirus disease of turkeys, usually around six to eleven weeks of age. It causes sudden death, sometimes with bloody droppings [38]:'),
    ...
    para('HE also knocks down the immune system, opening the door to secondary E. coli. A turkey flock with sudden deaths and big mottled spleens fits HE. On histology the lab finds intranuclear inclusion bodies in the spleen [38].'),

    spacer(40),
    para([{ text: 'Blackhead (Histomoniasis)', bold: true }]),
    para('Blackhead, or histomoniasis, is caused by the protozoan Histomonas meleagridis. It is devastating in turkeys, where mortality often runs 80 to 100% [39]. Chickens usually carry it quietly and pass it on. The lesions are so specific they confirm the diagnosis on sight:'),
    ...
    bullet([{ text: 'Liver:', bold: true }, { text: ' Round, sunken, target-like (bullseye) necrotic lesions up to a few centimeters across, green to tan. Together with the cecal cores, these are pathognomonic, meaning they confirm the diagnosis on their own [39].' }]),
    ...
    para('Blackhead has been re-emerging in recent years because there are no longer any approved drugs or vaccines to prevent or treat it, which keeps it a serious threat wherever turkeys are raised or share ground with chickens [39].'),
    pageBreak(),
